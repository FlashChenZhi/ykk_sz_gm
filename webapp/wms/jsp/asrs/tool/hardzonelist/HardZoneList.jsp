<%-- $Id: HardZoneList.jsp,v 1.1.1.1 2006/08/17 09:33:16 mori Exp $ --%>
        ...
        <TR>
          ...
          <TD bgColor=#b8b7d7>
            <TABLE cellSpacing=0 cellPadding=0 width=80 border=0>
              <TBODY>
              <TR>
                <TD><h:submitbutton id="btn_Query" templateKey="AST_Query"/></TD>
                <TD></TD>
                <TD></TD>
                <TD></TD>
                <TD></TD>
...
/*  2003/12/11  INSERT  okamura START  */
			...
			Shelf[] shelfAcc = (Shelf[])shelfhandle.find(shelfkeyacc);
			...
			if(shelfAcc.length > 0)
			{
				...
				{
					int whno = wToolFindUtil.getWarehouseNumber( shelfAcc[i].getWarehouseStationNumber() );
					String location = shelfAcc[i].getNumber( whno, shelfAcc[i].getBank(), shelfAcc[i].getBay(), shelfAcc[i].getLevel() );
...
/*  2003/12/11  DELETE  okamura END  */
				...
				for (int i=0 ; i < 10; i++)
				{
					zone = wToolFindUtil.getHardZoneName(i);
					if(zone.equals(""))
					{
						zone = "HARDZONE" + i;
					}
			...
			{
				if(wSelectedWareHouseStationNumber.equals(wareHouse[i].getNumber()))
				{
					//<jp> 倉庫No取得</jp>
					//<en> Get the warehouse no.</en>
					wWareHouseNumber = wareHouse[i].getWarehouseNumber();
					//<jp> バンクの一覧を取得</jp>
...
<%
		 			...
		 			String level_scale = fmt.format(allshelf[countshelf].getLevel());
...
<%
						...
						wBank  = allshelf[countshelf].getBank();
						wBay   = allshelf[countshelf].getBay();
						wLevel = allshelf[countshelf].getLevel();
/*  2003/12/11  INSERT  okamura START  */
						int whno = wToolFindUtil.getWarehouseNumber(allshelf[countshelf].getWarehouseStationNumber());
...
/*  2003/12/11  INSERT  okamura END  */
						...
						if ( ( wBank  == wBankNum[startbank] ) && 	
						     ...
						     ( wBay - 1 == k ) )			
						{
							String templocno = Shelf.getNumber(wWareHouseNumber, wBank, wBay, wLevel);
							String locInfo = DisplayText.formatLocation(
											Shelf.getNumber(wWareHouseNumber, wBank, wBay, wLevel) 
											);
...
/*  2003/12/11  INSERT  okamura END  */
								...
								if ( allshelf[countshelf].getHardZone() == 0 )
								{
										zone0_len++;
%>
...
<%
								...
								else if ( allshelf[countshelf].getHardZone() == 1 )
								{
										zone1_len++;
%>
...
<%
								...
								else if ( allshelf[countshelf].getHardZone() == 2 )
								{
										zone2_len++;
%>
...
<%
								...
								else if ( allshelf[countshelf].getHardZone() == 3 )
								{
										zone3_len++;
%>
...
<%
								...
								else if ( allshelf[countshelf].getHardZone() == 4 )
								{
										zone4_len++;
%>
...
<%
								...
								else if ( allshelf[countshelf].getHardZone() == 5 )
								{
										zone5_len++;
%>
...
<%
								...
								else if ( allshelf[countshelf].getHardZone() == 6 )
								{
										zone6_len++;
%>
...
<%
								...
								else if ( allshelf[countshelf].getHardZone() == 7 )
								{
										zone7_len++;
%>
...
<%
								...
								else if ( allshelf[countshelf].getHardZone() == 8 )
								{
										zone8_len++;
%>
...
<%
								...
								else if ( allshelf[countshelf].getHardZone() == 9 )
								{
										zone9_len++;
%>
...
%>
        ...
        <!--
            ...
            _setMessage('<%=jp.co.daifuku.bluedog.util.MessageResources.getText("6127005") %>',4);
        -->
        </script>
<%
	}
...
%>
        ...
        <!--
            var contextPath = "<%=request.getContextPath()%>";
            _setMessage('<%=jp.co.daifuku.bluedog.util.MessageResources.getText("6127006") %>',4);
        -->
        </script>
<%
	}
