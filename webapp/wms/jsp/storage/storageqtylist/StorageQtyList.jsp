<%-- $Id: StorageQtyList.jsp,v 1.1.1.1 2006/08/17 09:33:27 mori Exp $ --%>
...
<h:head>
  <script>
	function dynamicAction()
	{
		...
		if (_isValueInput('txt_StartStorageDate') && _isValueInput('txt_EndStorageDate'))
		{
			if (!_isCorrectRange(document.all('txt_StartStorageDate').value, document.all('txt_EndStorageDate').value))
			{
				document.all('txt_StartStorageDate').focus();
				// 6023106=開始入庫日は、終了入庫日より前の日付にしてください。
				_setMessage('<%= jp.co.daifuku.bluedog.util.MessageResources.getText("6023106") %>',2);
				return true;
			}
		}
		return false;
            ...
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <TR bgColor=#b8b7d7>
                ...
                <TD width="100%" bgColor=#b8b7d7><h:radiobutton id="rdo_Cpf_All" templateKey="W_Cpf_All"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                  ...
                  <h:radiobutton id="rdo_Cpf_AppointOff" templateKey="W_Cpf_AppointOff"/></TD></TR></TBODY></TABLE></TD></TR></TBODY></TABLE>
