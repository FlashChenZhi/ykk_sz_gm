 ...
 * Maker : Y.Okamura  <BR>
 * <BR>
 * マスタパッケージありの際に、予定外出庫設定画面より使用します。
 * <BR>
 ...
 * @author  $Author: mori $
 */
public class MasterNoPlanRetrievalSCH extends NoPlanRetrievalSCH
{
	 ...
	 * このクラスの初期化を行ないます。
	 */
	public MasterNoPlanRetrievalSCH()
	{
		super();
	}
	...
	public Parameter initFind( Connection conn, Parameter searchParam ) throws ReadWriteException, ScheduleException
	{
		...
		MasterStockControlParameter dispParam = new MasterStockControlParameter();
		if (tempParam != null)
		{
			dispParam.setConsignorCode(tempParam.getConsignorCode());
			dispParam.setConsignorName(tempParam.getConsignorName());
		}
		else
		{
			dispParam.setConsignorCode("");
			dispParam.setConsignorName("");
		}
		...
		{
			dispParam.setMergeType(MasterStockControlParameter.MERGE_TYPE_NONE);
		}
		else if (masterPrefs.getMergeType() == MasterPrefs.MERGE_REGIST_TYPE_OVERWRITE)
		{
			dispParam.setMergeType(MasterStockControlParameter.MERGE_TYPE_OVERWRITE);
		}
		else
		{
			dispParam.setMergeType(MasterStockControlParameter.MERGE_TYPE_FILL_EMPTY);
		}
		return dispParam;
	}
	 ...
	 * 
	 * 
	 * @see jp.co.daifuku.wms.stockcontrol.schedule.NoPlanRetrieval#query
	 */
	public Parameter[] query (Connection conn, Parameter searchParam) throws ReadWriteException, ScheduleException
	{
		if (searchParam instanceof MasterStockControlParameter)
		{
			MasterStockControlParameter param = (MasterStockControlParameter) searchParam;
			// 画面から補完を指定された場合にのみ補完処理を行う。
			if (param.getProcessType() == MasterStockControlParameter.PROCESS_TYPE_MERGE)
			{
				...
				if (plan != null)
				{
					MasterStockControlParameter tempParam = new MasterStockControlParameter();
				...
				MasterStockControlParameter[] rtParam = new MasterStockControlParameter[tempList.size()];
	...
	{
		// 入力エリアの内容
		MasterStockControlParameter[] param = (MasterStockControlParameter[]) startParams;
