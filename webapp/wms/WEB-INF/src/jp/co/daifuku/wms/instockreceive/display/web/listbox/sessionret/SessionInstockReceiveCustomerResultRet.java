 ...
 * <BR>
 * <B>1.検索処理(<CODE>SessionInstockReceiveCustomerResultRet(Connection, InstockReceiveParameter)</CODE>メソッド)<BR></B>
 * <DIR>
...
public class SessionInstockReceiveCustomerResultRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionInstockReceiveCustomerResultRet(Connection conn, InstockReceiveParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	...
	private InstockReceiveParameter[] convertToInStockParams(ResultView[] instockVIew)
	{
		...
		{	
			...
			for (int i = 0; i < instockVIew.length; i++)
			{
					...
					stParam[i].setCustomerCode(instockVIew[i].getCustomerCode());
					stParam[i].setCustomerName(instockVIew[i].getCustomerName1());
