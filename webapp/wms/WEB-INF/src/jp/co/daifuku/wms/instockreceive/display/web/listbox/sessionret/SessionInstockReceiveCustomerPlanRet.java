 ...
 * <BR>
 * <B>1.検索処理(<CODE>SessionInstockReceiveCustomerPlanRet(Connection, InstockReceiveParameter)</CODE>メソッド)<BR></B>
 * <DIR>
 ...
 * @author  $Author: mori $
 */
public class SessionInstockReceiveCustomerPlanRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionInstockReceiveCustomerPlanRet(Connection conn, InstockReceiveParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	...
	private InstockReceiveParameter[] convertToInStockParams(InstockPlan[] instockPlan)
	{
		...
		for (int i = 0; i < instockPlan.length; i++)
		{
				...
				stParam[i].setCustomerCode(instockPlan[i].getCustomerCode());
				stParam[i].setCustomerName(instockPlan[i].getCustomerName1());
		}
