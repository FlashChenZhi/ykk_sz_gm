// $Id: InstockReceiveQtyWriter.java,v 1.4 2006/12/13 08:56:45 suresh Exp $
 ...
 * Maker : Y.Okamura <BR>
 * <BR>
 * ｸﾛｽ/DC入荷実績リストの帳票用データファイルを作成し、印刷実行クラスを呼び出すためのクラスです。<BR>
 ...
 */
public class InstockReceiveQtyWriter extends CSVWriter
{
	// Class fields --------------------------------------------------
	...
	// Constructors --------------------------------------------------
	/**
	 * InstockReceiveQtyWriter オブジェクトを構築します。<BR>
	 * コネクションとロケールのセットを行います。<BR>
	 * @param conn データベースとのコネクションオブジェクト
	 */
	public InstockReceiveQtyWriter(Connection conn)
	{
		super(conn);
	}
	...
	/**
	 * ｸﾛｽ/DC入荷実績リスト用CSVファイルを作成し、印刷実行クラスを呼び出します。<BR>
	...
	public boolean startPrint()
	{
		...
		try
		{
			...
			viewKey.setItemCodeOrder(6, true);
			// 登録日
			viewKey.setRegistDateOrder(7, true);
			// 実績数
			viewKey.setResultQtyOrder(8, true);
			...
			// 出力ファイルを作成します。
			if (!createPrintWriter(FNAME_INSTOCK_QTY))
			{
				return false;
			}

            // 出力ファイルにヘッダーを作成
            wStrText.append(HEADER_INSTOCK_QTY);
				...
				for (int i = 0; i < resultView.length; i++)
				{
					...
					wStrText.append(
						 ReportOperation.format(
							DisplayUtil.getTcDcValue(resultView[i].getTcdcFlag())));
			...
			// UCXSingleを実行。（印刷実行）
			if (!executeUCX(JOBID_INSTOCK_QTY))
			{
	 ...
	 */
	private void setResultViewSearchKey(ResultViewSearchKey searchKey)
		throws ReadWriteException
	{
		// 検索キーをセットする。
		// 荷主コード
		...
		{
			searchKey.setTcdcFlag(wTcDcFlag);
		}
		else
		{
			// 何もセットされていないときはTC以外を検索
	...
	private String getConsignorName() throws ReadWriteException
	{
		...
		if (((ResultViewReportFinder) nameFinder).search(nameKey) > 0)
		{
			ResultView resultView[] =
				(ResultView[]) ((ResultViewReportFinder) nameFinder).getEntities(1);
