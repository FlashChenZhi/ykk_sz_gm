// $Id: CorrectCustomerMasterBusiness.java,v 1.1.1.1 2006/08/17 09:34:16 mori Exp $
 ...
 * @author  $Author: mori $
 */
public class CorrectCustomerMasterBusiness extends CorrectCustomerMaster implements WMSConstants
{
	// Class fields --------------------------------------------------
	// 遷移先アドレス
	// 荷主検索ポップアップアドレス
	...
	// 検索条件
	protected static final String searchKey = "2";
	...
	public void page_Load(ActionEvent e) throws Exception
	{
		...
		// 出荷先コードに入力値があるとき
		if (!StringUtil.isBlank(getViewState().getString("CustomerCode")))
		{
			txt_customerCode.setText(getViewState().getString("CustomerCode"));
		}
	 ...
	 * <BR>
	 * 概要:該当出荷先マスタ情報の修正・削除画面を表示します。<BR>
     * <BR>
     * <DIR>
	...
	public void btn_Next_Click(ActionEvent e) throws Exception
	{
		...
		try
		{
            ...
            if (schedule.nextCheck(conn,param))
            {
				...
				// ViewStateに出荷先コードをセットする
				getViewState().setString("CustomerCode", txt_customerCode.getText());
				// ViewStateに画面タイトルをセットする
				getViewState().setString("TITLE", lbl_SettingName.getResourceKey());
                // 画面遷移を行う
				forward("/master/correctcustomermaster/CorrectCustomerMaster2.do");
            }
            else
            {
