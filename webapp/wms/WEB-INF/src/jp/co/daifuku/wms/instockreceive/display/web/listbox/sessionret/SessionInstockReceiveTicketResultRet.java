 ...
 * <BR>
 * <B>1.検索処理(<CODE>SessionInstockReceiveTicketResultRet(Connection, InstockReceiveParameter)</CODE>メソッド)<BR></B>
 * <DIR>
...
public class SessionInstockReceiveTicketResultRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionInstockReceiveTicketResultRet(Connection conn, InstockReceiveParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	...
	private InstockReceiveParameter[] convertToInStockParams(ResultView[] instockVIew)
	{
		...
		{	
			...
			for (int i = 0; i < instockVIew.length; i++)
			{
					...
					stParam[i].setInstockTicketNo(instockVIew[i].getInstockTicketNo());
