 ...
 * <BR>
 * 出庫棚一覧リストボックス（移動ラック）用のデータを検索するためのクラスです。<BR>
 * 検索条件をパラメータとして受け取り、出庫棚一覧の検索を行います。<BR>
 ...
 * @author  $Author: mori $
 */
public class SessionIdmLocationRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionIdmLocationRet(Connection conn, IdmControlParameter stParam) throws Exception
	{
		wConn = conn ;		
		find(stParam) ;
	}
	...
	{	
		...
		if(!StringUtil.isBlank(stParam.getStartBankNo()))
		{
			...
			stKey.setLocationNo(iOpe.listSearchFormatIdmLocation(stParam.getStartBankNo(), stParam.getStartBayNo(), stParam.getStartLevelNo()), ">=") ;
		}
			
		if(!StringUtil.isBlank(stParam.getEndBankNo()))
		{
			IdmOperate iOpe = new IdmOperate();
			String chToLocation = iOpe.listSearchFormatIdmLocation(stParam.getEndBankNo(), stParam.getEndBayNo(), stParam.getEndLevelNo());
