//$Id: MasterShippingDataLoader.java,v 1.1.1.1 2006/08/17 09:34:20 mori Exp $
 ...
 * <BR>
 * <BR>
 * 出荷データ取込み時にマスタデータの補完処理を行うためのクラスです。<BR>
 ...
 * @author  $Author: mori $
 */
public class MasterShippingDataLoader extends jp.co.daifuku.wms.shipping.schedule.ShippingDataLoader
{
	// Class fields --------------------------------------------------
	...
	* 処理名
	*/
	private final String wProcessName = "MasterShippingDataLoader";
	...
	// Public methods ------------------------------------------------
	/**
	 * 出荷予定データ取り込み処理を行います。<BR>
	 * 処理は継承元の処理を呼び出します。
	 * 本メソッドでは、マスタデータ補完時に使用する
	 * オペレータクラスの生成を行います。
	 * 
	 * @see jp.co.daifuku.wms.shipping.schedule.ShippingDataLoader#load
	 */
		...
		throws ReadWriteException, ScheduleException
	{
		...
		wMerger = new ShippingPlanMGWrapper(conn);
	 ...
	 * </DIR>
	 * 
	 * @see jp.co.daifuku.wms.shipping.schedule.ShippingDataLoader#setParameter
	 */
	protected ShippingParameter setParameter(DataLoadCsvReader DRR, SystemParameter param, String pBatchNo)
		throws ReadWriteException, CsvIllegalDataException
	{
		ShippingParameter shipParam = super.setParameter(DRR, param, pBatchNo);

		ShippingPlan shipPlan = new ShippingPlan();

		// 入力値から補完処理を行う
		shipPlan.setConsignorCode(shipParam.getConsignorCode());
		shipPlan.setConsignorName(shipParam.getConsignorName());
		shipPlan.setCustomerCode(shipParam.getCustomerCode());
		shipPlan.setCustomerName1(shipParam.getCustomerName());
		shipPlan.setItemCode(shipParam.getItemCode());
		shipPlan.setItemName1(shipParam.getItemName());
		shipPlan.setSupplierCode(shipParam.getSupplierCode());
		shipPlan.setSupplierName1(shipParam.getSupplierName());
		shipPlan.setEnteringQty(shipParam.getEnteringQty());
		shipPlan.setBundleEnteringQty(shipParam.getBundleEnteringQty());
		shipPlan.setItf(shipParam.getITF());
		shipPlan.setBundleItf(shipParam.getBundleITF());

		wMerger.complete(shipPlan);

		// 補完結果を返却パラメータにセットする
		if (shipPlan != null)
		{
			shipParam.setConsignorName(shipPlan.getConsignorName());
			shipParam.setCustomerName(shipPlan.getCustomerName1());
			shipParam.setSupplierName(shipPlan.getSupplierName1());
			shipParam.setItemName(shipPlan.getItemName1());
			shipParam.setEnteringQty(shipPlan.getEnteringQty());
			shipParam.setBundleEnteringQty(shipPlan.getBundleEnteringQty());
			shipParam.setITF(shipPlan.getItf());
			shipParam.setBundleITF(shipPlan.getBundleItf());
	 ...
	 * 
	 * @see jp.co.daifuku.wms.shipping.schedule.ShippingDataLoader#check
	 */
	protected boolean check(
		ShippingParameter shipParam,
		WareNaviSystem wns,
		...
		throws ReadWriteException, ScheduleException
	{
		...
		masterParam = new MasterParameter();

		masterParam.setConsignorCode(shipParam.getConsignorCode());
		masterParam.setConsignorName(shipParam.getConsignorName());

		masterParam.setSupplierCode(shipParam.getSupplierCode());
		masterParam.setSupplierName(shipParam.getSupplierName());

		masterParam.setCustomerCode(shipParam.getCustomerCode());
		masterParam.setCustomerName(shipParam.getCustomerName());

		masterParam.setItemCode(shipParam.getItemCode());
		masterParam.setItemName(shipParam.getItemName());
		...
		try{
		    if(DRR.getEnable("SUPPLIER_CODE")){						
				...
				if (!StringUtil.isBlank(masterParam.getSupplierCode()) && iRet != MasterParameter.RET_OK)
				{
					if (iRet == MasterParameter.RET_NG)
					{
		...
		return super.check(shipParam, wns, DRR, errlist);
	...
	{
		// 処理の異常をログへ記録
		RmiMsgLogClient.write(Integer.parseInt(errorCode), wProcessName, msg);
