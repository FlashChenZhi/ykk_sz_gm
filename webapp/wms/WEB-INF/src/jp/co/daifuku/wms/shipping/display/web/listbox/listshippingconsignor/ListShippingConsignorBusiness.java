// $Id: ListShippingConsignorBusiness.java,v 1.1.1.1 2006/08/17 09:34:27 mori Exp $
 ...
 */
package jp.co.daifuku.wms.shipping.display.web.listbox.listshippingconsignor;
...
import jp.co.daifuku.wms.shipping.display.web.listbox.sessionret.SessionShippingPlanConsignorRet;
import jp.co.daifuku.wms.shipping.display.web.listbox.sessionret.SessionShippingResultConsignorRet;
 ...
 * @author  $Author: mori $
 */
public class ListShippingConsignorBusiness extends ListShippingConsignor implements WMSConstants
{
	// Class fields --------------------------------------------------
	/** 
	 * 出荷予定日の受け渡しに使用するキーです
	...
	public void page_Load(ActionEvent e) throws Exception
	{
		...
		// 検索する荷主が予定か実績か判断する
		if (searchconsignor.equals(ShippingParameter.SEARCHFLAG_PLAN))
		{
			// SessionShippingPlanConsignorRet インスタンス生成
			SessionShippingPlanConsignorRet listbox =
				new SessionShippingPlanConsignorRet(conn, param);
			// Sessionにlistboxを保持
			this.getSession().setAttribute("LISTBOX", listbox);
			setPlanList(listbox, "first");
		}
		else if (searchconsignor.equals(ShippingParameter.SEARCHFLAG_RESULT))
		{
			// SessionShippingResultConsignorRet インスタンス生成
			SessionShippingResultConsignorRet listbox =
				new SessionShippingResultConsignorRet(conn, param);
			// Sessionにlistboxを保持
			this.getSession().setAttribute("LISTBOX", listbox);
	 ...
	 * @throws Exception 全ての例外を報告します。
	 */
	private void setPlanList(SessionShippingPlanConsignorRet listbox, String actionName)
		throws Exception
	{
		// ページ情報をセット
		...
		// 検索結果を取得
		ShippingParameter[] splan = (ShippingParameter[]) listbox.getEntities();
		int len = 0;
		if (splan != null)
			len = splan.length;
		if (len > 0)
	 ...
	 * @throws Exception 全ての例外を報告します。
	 */
	private void setResultList(SessionShippingResultConsignorRet listbox, String actionName)
		throws Exception
	{
		// ページ情報をセット
		listbox.setActionName(actionName);
		...
		// 検索結果を取得
		ShippingParameter[] rview = (ShippingParameter[]) listbox.getEntities();
		int len = 0;
		if (rview != null)
			len = rview.length;
		if (len > 0)
	...
	public void pgr_D_Next(ActionEvent e) throws Exception
	{
		...
		if (flag.equals(ShippingParameter.SEARCHFLAG_PLAN))
		{
			// Sessionにlistboxを保持
			SessionShippingPlanConsignorRet listbox =
				(SessionShippingPlanConsignorRet) this.getSession().getAttribute("LISTBOX");
		...
		else if (flag.equals(ShippingParameter.SEARCHFLAG_RESULT))
		{
			// Sessionにlistboxを保持
			SessionShippingResultConsignorRet listbox =
				(SessionShippingResultConsignorRet) this.getSession().getAttribute("LISTBOX");
