//$Id: SortingHostSendViewFinder.java,v 1.1.1.1 2006/08/17 09:34:31 mori Exp $
 ...
 * @author  $Author: mori $
 */
public class SortingHostSendViewFinder extends HostSendViewReportFinder
{
	// Class filelds -----------------------------------------------
	 ...
	 */
	public SortingHostSendViewFinder(Connection conn)
	{
		super(conn);
	}
	 ...
	 * HostSendを検索し、取得します。
	 * @param key 検索のためのKey
	 * @param sql 結合テーブルのSQL文
	 * @return 検索結果の件数
	 * @throws ReadWriteException データベースとの接続で発生した例外をそのまま通知します。
	 */
	public int search(SortingHostSendViewSearchKey key, String sql) throws ReadWriteException
	{
		close();
		open();
		...
		try
		{
			...
			DEBUG.MSG("HANDLER", "SortingHostSendView Finder COUNT SQL[" + sqlcountstring + "]");
			...
			if ( count > 0 )
			{
				...
				DEBUG.MSG("HANDLER", "SortingHostSendView Finder SQL[" + sqlstring + "]");
		...
		catch (SQLException e)
		{
			...
			RmiMsgLogClient.write(new TraceHandler(6006002, e), "SortingHostSendViewFinder");
