...
import jp.co.daifuku.wms.instockreceive.report.TcInstockReceiveWriter;

/**
 * Designer : Y.Kubo <BR>
 * Maker : Y.Kubo <BR>
 * <BR>
 * TC入荷作業リスト発行処理を呼び出すためのクラスです。<BR>
 * 画面から入力された内容をパラメータとして受け取り、TC入荷作業リスト発行処理クラスを呼び出します。<BR>
 ...
 * <BR>
 * <DIR>
 *   画面から入力された内容をパラメータとして受け取り、TC入荷作業リスト発行処理クラスにパラメータを渡します。<BR>
 *   印刷内容の検索はWriterクラスで行います。<BR>
 *   印刷に成功した場合は、TC入荷作業リスト発行処理クラスからtrueを、失敗した場合はfalseを受け取ります。<BR>
 ...
 * @author  $Author: mori $
 */
public class TcInstockReceiveListSCH extends AbstractInstockReceiveSCH
{
	 ...
	 * このクラスの初期化を行ないます。
	 */
	public TcInstockReceiveListSCH()
	{
		wMessage = null;
	}
		...
		throws ReadWriteException, ScheduleException
	{
		...
		// 印刷クラスを作成します
		TcInstockReceiveWriter writer = createWriter(conn, parameter);
	...
	public int count(Connection conn, Parameter countParam) throws ReadWriteException, ScheduleException
	{
		...
		TcInstockReceiveWriter writer = createWriter(conn, param);
		// 対象件数を取得する
	 ...
	 * @return 印刷処理クラス
	 */
	protected TcInstockReceiveWriter createWriter(Connection conn, InstockReceiveParameter parameter)
	{

		// 印刷処理クラスに画面で入力された印刷条件をセットする。
		TcInstockReceiveWriter writer = new TcInstockReceiveWriter(conn);
