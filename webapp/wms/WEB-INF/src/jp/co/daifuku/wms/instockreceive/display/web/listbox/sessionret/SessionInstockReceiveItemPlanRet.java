 ...
 * <BR>
 * <B>1.検索処理(<CODE>SessionInstockReceiveItemPlanRet(Connection, InstockReceiveParameter)</CODE>メソッド)<BR></B>
 * <DIR>
 ...
 *     <tr><td></td><td>開始入荷受付日</td><td>：</td><td>FromPlanDate</td></tr>
 *     <tr><td></td><td>終了入荷受付日</td><td>：</td><td>ToPlanDate</td></tr>
 ...
 * <TR><TD>2004/11/02</TD><TD>Muneendra</TD><TD>created this class</TD></TR>
 * </TABLE>
 * <BR>
 * @version $Revision: 1.1.1.1 $, $Date: 2006/08/17 09:34:13 $
 * @author  $Author: mori $
 */
public class SessionInstockReceiveItemPlanRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionInstockReceiveItemPlanRet(Connection conn, InstockReceiveParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	...
	/**
	 * このクラスは InStockPlan エンティティ を InstockReceiveParameter パラメータに設定する。 <BR>
	 * 
	 * @param instockPlan 入荷予定情報
	 * @return InstockReceiveParameter[] 入荷情報をセットした<CODE>InstockReceiveParameter</CODE>クラス
	 */
	private InstockReceiveParameter[] convertToInStockParams(InstockPlan[] instockPlan)
	{
		...
		for (int i = 0; i < instockPlan.length; i++)
		{
				...
				stParam[i].setItemCode(instockPlan[i].getItemCode());
				stParam[i].setItemName(instockPlan[i].getItemName1());
		}
