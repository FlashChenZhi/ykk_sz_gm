// $Id: ListInstockReceiveSupplierBusiness.java,v 1.1.1.1 2006/08/17 09:34:12 mori Exp $
 ...
 */
package jp.co.daifuku.wms.instockreceive.display.web.listbox.listinstockreceivesupplier;
...
import jp.co.daifuku.wms.instockreceive.display.web.listbox.sessionret.SessionInstockReceiveSupplierPlanRet;
import jp.co.daifuku.wms.instockreceive.display.web.listbox.sessionret.SessionInstockReceiveSupplierResultRet;
 ...
 * @author  $Author: mori $
 */
public class ListInstockReceiveSupplierBusiness extends ListInstockReceiveSupplier implements WMSConstants
{
	// Class fields --------------------------------------------------
	/** 
	...
	public void page_Load(ActionEvent e) throws Exception
	{
		...
		if (searchsupplier.equals(InstockReceiveParameter.SEARCHFLAG_PLAN))
		{
			...
			SessionInstockReceiveSupplierPlanRet listbox = new SessionInstockReceiveSupplierPlanRet(conn, param);
			//Sessionにlistboxを保持
		...
		{
			//SessionShippingResultSupplierRet インスタンス生成
			SessionInstockReceiveSupplierResultRet listbox = new SessionInstockReceiveSupplierResultRet(conn, param);
			//Sessionにlistboxを保持
			this.getSession().setAttribute("LISTBOX", listbox);
	 ...
	 * @throws Exception 全ての例外を報告します。
	 */
	private void setPlanList(SessionInstockReceiveSupplierPlanRet listbox, String actionName) throws Exception
	{
		// ページ情報をセット
		...
		if (len > 0)
		{
			...
			// 行を全て削除
			lst_InstkSupplierSrch.clearRow();

			for (int i = 0; i < len; i++)
			{
				// 最終行を取得
				int count = lst_InstkSupplierSrch.getMaxRows();
				// 行追加
				lst_InstkSupplierSrch.addRow();

				// 最終行に移動
				lst_InstkSupplierSrch.setCurrentRow(count);
				lst_InstkSupplierSrch.setValue(1, Integer.toString(count + listbox.getCurrent()));
				lst_InstkSupplierSrch.setValue(2, splan[i].getSupplierCode());
				lst_InstkSupplierSrch.setValue(3, splan[i].getSupplierName());
			}
		}
		else
		{
			...
			// ヘッダーを隠します
			lst_InstkSupplierSrch.setVisible(false);
			// エラーメッセージ表示
	 ...
	 * @throws Exception 全ての例外を報告します。
	 */
	private void setResultList(SessionInstockReceiveSupplierResultRet listbox, String actionName)
		throws Exception
	{
		// ページ情報をセット
		listbox.setActionName(actionName);
		...
		if (len > 0)
		{
			...
			for (int i = 0; i < len; i++)
			{
				...
				lst_InstkSupplierSrch.setCurrentRow(count);
				lst_InstkSupplierSrch.setValue(1, Integer.toString(count + listbox.getCurrent()));
				lst_InstkSupplierSrch.setValue(2, rview[i].getSupplierCode());
				lst_InstkSupplierSrch.setValue(3, rview[i].getSupplierName());
			}
		}
		else
		{
	 ...
	 * @throws Exception 
	 */
	public void lst_InstkSupplierSrch_EnterKey(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_InstkSupplierSrch_TabKey(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_InstkSupplierSrch_InputComplete(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_InstkSupplierSrch_ColumClick(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_InstkSupplierSrch_Server(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_InstkSupplierSrch_Change(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 全ての例外を報告します。
	 */
	public void lst_InstkSupplierSrch_Click(ActionEvent e) throws Exception
	{
		// 現在の行をセット
		lst_InstkSupplierSrch.setCurrentRow(lst_InstkSupplierSrch.getActiveRow());
		lst_InstkSupplierSrch.getValue(1);

		// 親画面に返却するパラメータセット
		ForwardParameters param = new ForwardParameters();
		// 仕入先コード
		param.setParameter(SUPPLIERCODE_KEY, lst_InstkSupplierSrch.getValue(2));
		// 仕入先名称
		param.setParameter(SUPPLIERNAME_KEY, lst_InstkSupplierSrch.getValue(3));
	...
	public void pgr_D_Next(ActionEvent e) throws Exception
	{
		...
		{
			// Sessionにlistboxを保持
			SessionInstockReceiveSupplierPlanRet listbox =
				(SessionInstockReceiveSupplierPlanRet) this.getSession().getAttribute("LISTBOX");
			setPlanList(listbox, "next");
		}
		else if (flag.equals(InstockReceiveParameter.SEARCHFLAG_RESULT))
		{
			// Sessionにlistboxを保持
			SessionInstockReceiveSupplierResultRet listbox =
				(SessionInstockReceiveSupplierResultRet) this.getSession().getAttribute("LISTBOX");
