// $Id: InstockReceiveQtyListBusiness.java,v 1.1.1.1 2006/08/17 09:34:11 mori Exp $
 ...
 */
package jp.co.daifuku.wms.instockreceive.display.web.instockreceiveqtylist;
...
import jp.co.daifuku.wms.instockreceive.display.web.listbox.listinstockreceiveqtylist.ListInstockReceiveQtyListBusiness;
...
import jp.co.daifuku.wms.instockreceive.schedule.InstockReceiveQtyListSCH;

/**
 * Designer : K.Mukai <BR>
 * Maker : T.Yoshino<BR>
 * クロス／ＤＣ 入荷実績リスト発行クラスです<BR>
 * クロス／ＤＣ 入荷実績リスト発行処理を行うスケジュールにパラメータを引き渡します。<BR>
 ...
 *  スケジュールから検索結果を受け取り、クロス／ＤＣ 入荷実績リスト一覧画面を呼出します<BR>
 *  <BR>
 *  [パラメータ] *必須入力<BR>
 *  <DIR>
 *   荷主コード*<BR>
 ...
 * @author  $Author: mori $
 */
public class InstockReceiveQtyListBusiness extends InstockReceiveQtyList implements WMSConstants
{
	// Class fields --------------------------------------------------
	...
	public void page_ConfirmBack(ActionEvent e) throws Exception
	{
		...
		try
		{
			...
			// スケジュールを開始する
			WmsScheduler schedule = new InstockReceiveQtyListSCH();
	...
	public void btn_PDisplay_Click(ActionEvent e) throws Exception
	{
		...
		if (rdo_CrossDCFlagAll.getChecked())
		{
			forwardParam.setParameter(ListInstockReceiveQtyListBusiness.CROSSDCFLAG_KEY, InstockReceiveParameter.TCDC_FLAG_ALL);
		}
		else if (rdo_CrossDCFlagCross.getChecked())
		{
			forwardParam.setParameter(ListInstockReceiveQtyListBusiness.CROSSDCFLAG_KEY, InstockReceiveParameter.TCDC_FLAG_CROSSTC);
		}
		else if (rdo_CrossDCFlagDC.getChecked())
		{
			forwardParam.setParameter(ListInstockReceiveQtyListBusiness.CROSSDCFLAG_KEY, InstockReceiveParameter.TCDC_FLAG_DC);
		}
		// クロス／ＤＣ入荷実績リスト一覧リストボックスを表示する。
		redirect("/instockreceive/listbox/listinstockreceiveqtylist/ListInstockReceiveQtyList.do", forwardParam, "/progress.do");
	}
