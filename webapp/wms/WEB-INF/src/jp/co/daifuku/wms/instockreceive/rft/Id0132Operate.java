// $Id: Id0132Operate.java,v 1.1.1.1 2006/08/17 09:34:14 mori Exp $
 ...
 * Maker :   <BR>
 * <BR>
 * 仕入先単位入荷検品開始要求に対するデータ処理を行ないます。<BR>
 * 検品データの取得し、そのデータを作業中に更新します。また、検品予定ファイルの作成を行います。<BR>
 * Id0132Processから呼び出されるビジネスロジックが実装されます。<BR>
 * <BR>
 * ！注意！<BR>
 ...
 * @author  $Author: mori $
 */
public class Id0132Operate extends IdOperate
{
	// Class fields----------------------------------------------------
	/**
	 * 処理名（登録処理名、最終更新処理名用）
	...
	// Public methods ------------------------------------------------
	/**
	 * 仕入先単位入荷検品開始要求に対するデータを取得し、作業可能であれば、そのデータの状態を作業中にします。<BR>
	 ...
	 * <DIR>
	 *   荷主コード、入荷予定日、仕入先コード = パラメータより取得<BR>
	 *   作業区分 = 入荷<BR>
	...
	*/
	public WorkingInformation[] startReceivingBySupplier(
		...
		throws NotFoundException, InvalidDefineException, ReadWriteException, LockTimeOutException, IllegalAccessException, OverflowException
	{
		...
		pskey = instockOperate.getConditionOfInstockBySupplier(
		        planDate,
		        consignorCode,
		        supplierCode,
		        null,
		...
		if (workinfo.length == 0)
		{
			...
			// DCまたはクロスTC
			pskey.setTcdcFlag(WorkingInformation.TCDC_FLAG_DC, "=", "(", "", "OR");
			pskey.setTcdcFlag(WorkingInformation.TCDC_FLAG_CROSSTC, "=", "", ")", "AND");
			// 状態フラグが「削除」以外を対象にする。
			...
			if (workinfo.length == 0)
			{
				NotFoundException e = new NotFoundException(RFTId5132.ANS_CODE_NULL);
				throw e;
			}
			// 該当データに、作業中or開始済データが見つかった時は、状態フラグを 1:他端末作業中 で例外をスロー
			for (int i = 0; i < workinfo.length; i++)
			{
				...
				{
					NotFoundException e = new NotFoundException(RFTId5132.ANS_CODE_WORKING);
					throw e;
				}
			}
			// 該当データに、作業中or開始済データが見つからなかった時は、状態フラグを 2:作業完了済み で例外をスロー
			NotFoundException e = new NotFoundException(RFTId5132.ANS_CODE_COMPLETION);
			throw e;
		}
