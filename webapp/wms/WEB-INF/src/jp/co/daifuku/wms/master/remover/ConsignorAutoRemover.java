// $Id: ConsignorAutoRemover.java,v 1.1.1.1 2006/08/17 09:34:19 mori Exp $
...
public class ConsignorAutoRemover
		extends AbstractAutoRemover
{
	...
	//------------------------------------------------------------
	private ConsignorHandler p_handler = null ;
	 ...
	 */
	public ConsignorAutoRemover(Connection conn)
	...
	protected DatabaseHandler getDBHandler(Connection conn)
	{
		if (p_handler == null)
		{
			p_handler = new ConsignorHandler(conn) ;
		}
		return p_handler ;
	}
	 ...
	 * 実績を検索し、最終更新日時で荷主マスタの使用日時を更新します。
	 * 
			...
			throws ReadWriteException
	{
		...
		// DNRESULTと荷主コードで結合
		String[] relationalSql = { Result.CONSIGNORCODE.getName() };
		
		// データ検索
		if(rFinder.searchMaster(rKey, Consignor.TABLE_NAME, relationalSql) <= 0)
		{
			return;
		}
		...
		while (rFinder.isNext())
		{
			...
			{
				// 荷主マスタ更新
				updateUsed(handler, result[i].getConsignorCode(), result[i].getLastUpdateDate());
			}
		}
	}
	 ...
	 * @throws ReadWriteException データベースへのアクセスに失敗したときスローされます。
	 */
	protected void updateUsed(DatabaseHandler handler, String code, Date lastUsedDate)
			throws ReadWriteException
	{
	 ...
	 * @throws ReadWriteException データベースへのアクセスに失敗したときスローされます。
	 */
	protected void updateUsed(DatabaseHandler handler, String code)
			throws ReadWriteException
	{
		updateUsed(handler, code, new Date());
	}
	...
	{
		return "$Id: ConsignorAutoRemover.java,v 1.1.1.1 2006/08/17 09:34:19 mori Exp $" ;
	}
