 ...
 * Maker : M.Takeuchi <BR>
 * <BR>
 * 予定外入庫設定処理を行うためのクラスです。 <BR>
 * 画面から入力された内容をパラメータとして受け取り、予定外入庫設定処理を行います。 <BR>
 ...
 * <BR>
 * <DIR>
 *   ためうちエリアに表示されている内容をパラメータとして受け取り、予定外入庫設定処理を行います。 <BR>
 *   処理完了時にパラメータの予定外入庫作業リスト発行区分がtrueの場合、予定外入庫作業リスト発行クラスを起動します。<BR>
 ...
 * @author  $Author: mori $
 */
public class MasterNoPlanStorageSCH extends NoPlanStorageSCH
{
	//	Class variables -----------------------------------------------
	/**
	 * クラス名(予定外入庫)
	 */
	public static String PROCESSNAME = "MasterNoPlanStorageSCH";
	 ...
	 */
	public MasterNoPlanStorageSCH(Connection conn) throws ReadWriteException, ScheduleException
	{
		wMessage = null;
