 ...
 * Maker : Y.Okamura  <BR>
 * <BR>
 * マスタパッケージありの際に、IDM予定外出庫設定画面より使用します。
 * <BR>
 ...
 * @author  $Author: mori $
 */
public class MasterIdmNoPlanRetrievalSCH extends IdmNoPlanRetrievalSCH
{
	// Class variables -----------------------------------------------
	 ...
	 * このクラスの初期化を行ないます。
	 */
	public MasterIdmNoPlanRetrievalSCH()
	{
		super();
	}
		...
		throws ReadWriteException, ScheduleException
	{
		...
		MasterIdmControlParameter dispParam = new MasterIdmControlParameter();
		if (tempParam != null)
		{
		...
		{
			dispParam.setMergeType(MasterIdmControlParameter.MERGE_TYPE_NONE);
		}
		else if (masterPrefs.getMergeType() == MasterPrefs.MERGE_REGIST_TYPE_OVERWRITE)
		{
			dispParam.setMergeType(MasterIdmControlParameter.MERGE_TYPE_OVERWRITE);
		}
		else
		{
			dispParam.setMergeType(MasterIdmControlParameter.MERGE_TYPE_FILL_EMPTY);
		}
		return dispParam;
	}
	 ...
	 * 
	 * 
	 * @see jp.co.daifuku.wms.idm.schedule.IdmNoPlanRetrieval#query
	 */
	public Parameter[] query (Connection conn, Parameter searchParam) throws ReadWriteException, ScheduleException
	{
		if (searchParam instanceof MasterIdmControlParameter)
		{
			MasterIdmControlParameter param = (MasterIdmControlParameter) searchParam;
			// 画面から補完を指定された場合にのみ補完処理を行う。
			if (param.getProcessType() == MasterIdmControlParameter.PROCESS_TYPE_MERGE)
			{
				...
				if (plan != null)
				{
					MasterIdmControlParameter tempParam = new MasterIdmControlParameter();
				...
				MasterIdmControlParameter[] rtParam = new MasterIdmControlParameter[tempList.size()];
	...
	{
		// 入力エリアの内容
		MasterIdmControlParameter[] param = (MasterIdmControlParameter[]) startParams;
