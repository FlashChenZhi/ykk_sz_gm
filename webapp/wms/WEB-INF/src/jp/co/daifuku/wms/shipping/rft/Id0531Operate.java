// $Id: Id0531Operate.java,v 1.1.1.1 2006/08/17 09:34:30 mori Exp $
 ...
 * Id0531Processから呼び出されるビジネスロジックが実装されます。<BR>
 * <BR>
 * <TABLE BORDER="1" CELLPADDING="3" CELLSPACING="0">
 * <TR BGCOLOR="#CCCCFF" CLASS="TableHeadingColor"><TD>Date</TD><TD>Name</TD><TD>Comment</TD></TR>
 * <TR><TD>2004/10/25</TD><TD>T.Yamashita</TD><TD>created this class</TD></TR>
 * </TABLE>
 * <BR>
 * @version $Revision: 1.1.1.1 $, $Date: 2006/08/17 09:34:30 $
 * @author  $Author: mori $
 */
public class Id0531Operate extends IdOperate
{
	// Class variables -----------------------------------------------
	private static final String PROCESS_NAME = "ID0531";
	private static final String START_PROCESS_NAME = "ID0530";
	private static final String CLASS_NAME = "Id0531Operate";
	// エラー詳細を保持する変数
	private String errDetails = RFTId5531.ErrorDetails.NORMAL;
	 ...
	 */
	public Id0531Operate(Connection conn)
	{
		super();
		wConn = conn;
	}
	...
	// Public methods ------------------------------------------------
	/**
	 * 商品単位出荷検品実績データ(ID0231)の処理を行います。<BR>
	...
	*/
		...
		int missScanCnt)
	{
		try
		{
			...
			// 受付区分が9:キャンセルの場合
			if (completionFlag.equals(RFTId0531.COMPLETION_FLAG_CANCEL))
			{
			...
			{
				// 完了フラグに応じて完了処理を行う
				if (! completionFlag.equals(RFTId0531.COMPLETION_FLAG_RESERVE))
				{
					// 正常または欠品の場合、完了処理を行う
					shippingCompleteOnCustomer(
						consignorCode,
						planDate,
						customerCode,
						janCode,
		...
		catch (NotFoundException e)
		{
			...
			errDetails = RFTId5531.ErrorDetails.NULL;
			// 応答フラグ：エラー
			return RFTId5531.ANS_CODE_ERROR;
		}
		...
		catch (UpdateByOtherTerminalException e)
		{
			...
			errDetails = RFTId5531.ErrorDetails.UPDATE_FINISH;
			// 応答フラグ：エラー
			return RFTId5531.ANS_CODE_ERROR;
		}
		// データアクセスでエラーがあった場合
		...
		catch (InvalidStatusException e)
		{
			...
			errDetails = RFTId5531.ErrorDetails.DB_INVALID_VALUE;
			// 応答フラグ：エラー
			return RFTId5531.ANS_CODE_ERROR;
		}
		catch (DataExistsException e)
		{
			...
			errDetails = RFTId5531.ErrorDetails.DB_UNIQUE_KEY_ERROR;
			// 応答フラグ：エラー
			return RFTId5531.ANS_CODE_ERROR;
		}
		// その他のエラーがあった場合
		...
		return RFTId5531.ANS_CODE_NORMAL;
	}

	/**
	 * 商品単位出荷検品実績送信 [確定処理]<BR>
	 * 出荷実績情報について、以下の確定処理を実行します。<BR>
	 * 
	 *  作業情報の更新。該当するデータを検索し、状態フラグを完了に更新し、
	 *  実績数などの項目をセットします。<BR>
	 *  出荷予定情報の更新。該当するデータを検索し、状態フラグを完了に更新し、
	 *  実績数などの項目をセットします。<BR>
	 ...
	 *      <TR><TD>最終更新処理名</TD>	<TD>"Id0531"</TD></TR>
	 *    </TABLE>
	...
	*/
	public void shippingCompleteOnCustomer(
		...
		int missScanCnt)
		throws
			InvalidStatusException, NotFoundException, InvalidDefineException, DataExistsException, 
			UpdateByOtherTerminalException, IllegalAccessException, ReadWriteException, LockTimeOutException
	{
		ShippingWorkOperator shippingWorkOperator;
		try
		{
		...
		String[] planUkeyList =
			shippingWorkOperator.lockUpdateData(consignorCode, planDate, customerCode, janCode, rftNo, workerCode);
	...
	/**
	 * 商品単位出荷検品実績送信 [保留処理]<BR>
	 * 出荷実績情報について、以下の確定処理を実行します。<BR>
	 * 
	 *  作業情報の更新と新規作業情報の作成。該当するデータを検索し、完了した分だけ完了にし、
	 *  完了していない分を未開始にします。<BR>
	 *  (1レコード内の一部が検品作業済みでの時は、未開始のレコードを新規作成します。)<BR>
	 *  作業情報の更新。該当するデータを検索し、状態フラグを完了又は一部完了に更新し、
	 *  実績数などの項目をセットします。<BR>
	 *  在庫情報の更新。該当するデータを検索し、在庫数と引当数を減算します。<BR>
	 *  実績送信情報の作成。検品完了した実績を作成します。<BR>
	 *  作業者実績情報の更新。該当するデータを検索し、作業数、作業回数を加算します。<BR>
	 ...
	 *    </UL>
	 *   複数レコードが該当する事を考慮する必要があります。<BR>
	 *   実際に検品した実績数を、該当レコードの作業実績数に分配します。
	 *   分配する数量は「作業可能数」です。<BR>
	 *   過剰の場合は、最後のレコードに分配します。<BR>
	 *   不足の場合は、そのレコードの実績が0になる場合と、実績が1以上（作業可能数よりは小さい値）に
	 *   なる場合で処理が分かれます。<BR>
	 * 
	 *     実績が0になる場合<BR>
	 ...
	 *     実績が1以上になる場合<BR>
	 *         ...レコードを分割します。<BR>
	 *            元レコードは、作業実績数に実績数をセットします。
	 *            また、保留数に可能数-実績数をセットします。<BR>
	 *            状態フラグを完了にセットします。賞味期限、処理名をセットします。<BR>
	 *            状態フラグを完了にした分の実績送信情報の処理を行います。<BR>
	 *            新レコードは、作業Noを新規に取得してセットします。<BR>
	 *            作業可能数と予定数は、分割前の作業可能数から実績数を減算した値（残りの作業）を
	 *            セットします。作業実績数は0をセットします。<BR>
	 * </DIR>
	 * <BR>
	 * 出荷予定情報の更新<BR>
	 * <DIR>
	...
	public void shippingSuspendOnCustomer(
		...
		int missScanCnt)
		throws
			InvalidStatusException, NotFoundException, InvalidDefineException, DataExistsException, UpdateByOtherTerminalException, IllegalAccessException, ReadWriteException, LockTimeOutException
	{
		ShippingWorkOperator shippingWorkOperator;
		try
		{
	 ...
	 *      <TR><TD>作業欠品数</TD>		<TD>電文の実績数が作業可能数に満たない場合、不足分をセットする</TD></TR>
	 *      <TR><TD>賞味期限</TD>		<TD>電文の賞味期限</TD></TR>
	 *      <TR><TD>最終更新処理名</TD>	<TD>"Id0063"</TD></TR>
	 *    </TABLE>
	 *   複数レコードが該当する事を考慮する必要があります。<BR>
	 *   実際に検品した実績数を、該当レコードの作業実績数に分配します。分配する数量は「作業可能数」です。<BR>
	 *   過剰の場合は、最後のレコードに分配します。<BR>
	...
	protected boolean updateWorkingInformation(
		...
		throws
			...
			ReadWriteException,
			DataExistsException,
			UpdateByOtherTerminalException, IllegalAccessException
	{
		...
		shippingOperate.checkCollectData(workingInformation, wi, START_PROCESS_NAME);

		// 出荷実績情報の実績数を保持
		int workQty = workingInformation.getResultQty();
		...
		for (int j = 0; j < wi.length; j ++)
		{
			...
			if (workQty > 0 || isShortage)
			{
				...
				akey.updateResultUseByDate(workingInformation.getResultUseByDate());
				// 現在の作業情報エンティティの賞味期限をセット
				wi[j].setResultUseByDate(workingInformation.getResultUseByDate());
