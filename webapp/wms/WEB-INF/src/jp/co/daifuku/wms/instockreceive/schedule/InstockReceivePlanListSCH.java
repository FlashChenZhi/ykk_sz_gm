...
import jp.co.daifuku.wms.instockreceive.report.InstockReceivePlanWriter;
/**
 * Designer : Muneendra <BR>
 * Maker : Muneendra <BR>
 * 
 *
 * <CODE>InstockReceivePlanListSCH</CODE>クラスは、入荷予定情報のリスト発行を行います。<BR>
 * 画面から入力された内容をパラメータとして受け取り、クロス/DC入荷予定リスト発行処理を行います。<BR>
 ...
 * <BR>
 * 	＜印刷処理＞<BR>
 * 	1.パラメータにセットされた値を<CODE>InstockReceivePlanWriter</CODE>クラスにセットします。<BR>
 * 	2.<CODE>InstockReceivePlanWriter</CODE>クラスを使用して、クロス/DC入荷予定リストを発行します。<BR>
 * 
 * </DIR>
 * <BR>
...
public class InstockReceivePlanListSCH extends AbstractInstockReceiveSCH
{
	 ...
	 * このクラスの初期化を行ないます。
	 */
	public InstockReceivePlanListSCH()
	{
		wMessage = null;
	}
	...
	public Parameter initFind(Connection conn, Parameter searchParam)throws ReadWriteException, ScheduleException
	{
		...
		try
		{
			...
			// TC/DC区分(ＤＣ、クロスドック)
			String[] tcdcflag = { InstockPlan.TCDC_FLAG_DC, InstockPlan.TCDC_FLAG_CROSSTC };
	...
	/**
	 * 画面から入力された内容をパラメータとして受け取り、クロス/DC入荷予定リスト発行スケジュールを開始します。<BR>
	 * ためうちからの設定など、複数データの入力を想定しているのでパラメータは配列で受け取ります。<BR>
	 * 詳しい動作はクラス説明の項を参照してください。<BR>
	 * <CODE>InStockPlanWriter</CODE>クラスを使用してクロス/DC入荷予定リストの印刷処理を行います。<BR>
	 * 
	 * @param conn データベースとのコネクションを保持するインスタンス。
	 * @param startParams 設定内容を持つ<CODE>InStockPlanWriter</CODE>クラスのインスタンスの配列。
		...
		throws ReadWriteException, ScheduleException
	{
		...
		// 印刷クラスを作成します
		InstockReceivePlanWriter writer = createWriter(conn, parameter);
	...
	public int count(Connection conn, Parameter countParam) throws ReadWriteException, ScheduleException
	{
		...
		InstockReceivePlanWriter writer = createWriter(conn, param);
		// 対象件数を取得する
	 ...
	 * @return 印刷処理クラス
	 */
	protected InstockReceivePlanWriter createWriter(Connection conn, InstockReceiveParameter parameter)
	{
		// 印刷処理クラスを生成する
		InstockReceivePlanWriter writer = new InstockReceivePlanWriter(conn);
		...
		writer.setTcdcFlag(parameter.getTcdcFlag());
