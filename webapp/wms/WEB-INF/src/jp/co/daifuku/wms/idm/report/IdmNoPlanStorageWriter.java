// $Id: IdmNoPlanStorageWriter.java,v 1.4 2006/12/13 08:55:41 suresh Exp $
 ...
 * Maker : K.Toda <BR>
 *
 * <CODE>IdmNoPlanStorageWriter</CODE>?????????????????????????????????????????? <BR>
 * <CODE>IdmNoPlanStorageSCH</CODE>????????????????????????(DNHOSTSEND)????? <BR>
 * ???????????????????????????????? <BR>
 * ?????????????????? <BR>
 * <BR>
 * ??????????????(<CODE>startPrint()</CODE>????) <BR>
 * <DIR>
 *	1.<CODE>IdmNoPlanStorageSCH</CODE>????????????????????(DNHOSTSEND)?????????? <BR>
 *	2.???1???????????????????0???????false?????????? <BR>
 *	3.??????????? <BR>
 *  4.?????????????true?????? <BR>
 *    ?????????????????false?????? <BR>
 ...
 */
public class IdmNoPlanStorageWriter extends CSVWriter
{
	// Class fields --------------------------------------------------
	...
	// Constructors --------------------------------------------------
	/**
	 * IdmNoPlanStorageWriter ?????????????
	 *
	 * @param conn<CODE>Connection</CODE>
	 * @param locale<CODE>Locale</CODE>
	 */
	public IdmNoPlanStorageWriter(Connection conn)
	{
		super(conn);
	}
	...
	public boolean startPrint()
	{
		...
		try
		{
			...
			searchKey.setLocationNoOrder(2, true);
			...
			// ?????????
			if (!createPrintWriter(FNAME_IDMSTORAGE))
			{
				return false;
			}

            //append header
            wStrText.append(HEADER_IDMSTORAGE);
			...
			while (reportFinder.isNext())
			{
				...
				for (int i = 0; i < hostSend.length; i++)
				{
					wStrText.append(re + "");
					// ?????
					wStrText.append(ReportOperation.format(hostSend[i].getConsignorCode())
						+ tb);
					// ????
					wStrText.append(ReportOperation.format(hostSend[i].getConsignorName())
						+ tb);
					// ???
					wStrText.append(ReportOperation.format(hostSend[i].getWorkDate())
						+ tb);
					// ??
					wStrText.append(ReportOperation.format(hostSend[i].getLocationNo())
						+ tb);
					// ?????
					wStrText.append(ReportOperation.format(hostSend[i].getItemCode())
						+ tb);
					// ????
					wStrText.append(ReportOperation.format(hostSend[i].getItemName1())
						+ tb);
					// ???/?????
					wStrText.append(ReportOperation.format(
							DisplayUtil.getPieceCaseValue(
								hostSend[i].getCasePieceFlag()))
						+ tb);
					// ?????
					wStrText.append(ReportOperation.format(
							Integer.toString(hostSend[i].getEnteringQty()))
						+ tb);
					// ?????
					wStrText.append(ReportOperation.format(
							Integer.toString(
								hostSend[i].getBundleEnteringQty()))
						+ tb);
					//?????
					wStrText.append(DisplayUtil.getCaseQty(hostSend[i].getResultQty(), hostSend[i].getEnteringQty(), hostSend[i].getCasePieceFlag())
						+ tb);
					//?????
					wStrText.append(DisplayUtil.getPieceQty(hostSend[i].getResultQty(), hostSend[i].getEnteringQty(), hostSend[i].getCasePieceFlag())
						+ tb);
					// ???ITF
					wStrText.append(ReportOperation.format(hostSend[i].getItf())
						+ tb);
					// ???ITF
					wStrText.append(ReportOperation.format(hostSend[i].getBundleItf())
						+ tb);
					// ????
			...
			wPWriter.close();
			// UCXSingle???
			if (!executeUCX(JOBID_IDMSTORAGE))
			{
				// ??????????????????????
				return false;
			}
