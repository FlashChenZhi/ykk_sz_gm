// $Id: ItemAutoRemover.java,v 1.1.1.1 2006/08/17 09:34:19 mori Exp $
		...
		extends AbstractAutoRemover
{
	...
	//------------------------------------------------------------
	private ItemHandler p_handler = null ;
	 ...
	 */
	public ItemAutoRemover(Connection conn)
	...
	protected DatabaseHandler getDBHandler(Connection conn)
	{
		if (p_handler == null)
		{
			p_handler = new ItemHandler(conn) ;
		}
		return p_handler ;
	}
	 ...
	 * 実績を検索し、最終更新日時で商品マスタの使用日時を更新します。
	 * 
			...
			throws ReadWriteException
	{
		...
		// 商品コード
		rKey.setItemCodeGroup(2);

		// DNRESULTと荷主コード、商品コードで結合
		String[] relationalSql = { Result.CONSIGNORCODE.getName(), Result.ITEMCODE.getName() };
		
		// データ検索
		if(rFinder.searchMaster(rKey, Item.TABLE_NAME, relationalSql) <= 0)
		{
			return;
		}
		...
		while (rFinder.isNext())
		{
			...
			{
				// 商品マスタ更新
				updateUsed(handler, result[i].getConsignorCode(), result[i].getItemCode(), result[i].getLastUpdateDate());
			}
		}
	}
	...
	{
		return "$Id: ItemAutoRemover.java,v 1.1.1.1 2006/08/17 09:34:19 mori Exp $" ;
	}	
}
