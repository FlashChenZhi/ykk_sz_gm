 ...
 * <BR>
 * TC入荷実績照会処理を行うためのクラスです。 <BR>
 * 画面から入力された内容をパラメータとして受け取り、TC入荷実績照会処理を行います。 <BR>
 ...
 *   作業日、予定日、仕入先コード、仕入先名称、出荷先コード、出荷先名称、伝票№、行№、商品コード、商品名称、
 *   ケース入数、ボール入数、作業予定ケース数、作業予定ピース数、実績ケース数、実績ピース数、欠品ケース数、欠品ピース数、賞味期限、
 *   ケースＩＴＦ、ボールＩＴＦ、作業者コード、作業者名を表示します。 <BR>
 ...
 * @version 2004/11/02
 */
public class TcInstockReceiveQtyInquirySCH extends AbstractInstockReceiveSCH
{
	//  Constants----------------------------------------------------
	 ...
	 * このクラスの初期化を行ないます。
	 */
	public TcInstockReceiveQtyInquirySCH()
	{
		wMessage = null;
	}
	 ...
	 * @param searchParam
	 *            表示データ取得条件を持つ <CODE>InstockReceiveParameter</CODE>
	 *            クラスのインスタンス。 <CODE>InstockReceiveParameter</CODE>
		...
		throws ReadWriteException, ScheduleException
	{
		...
		// 表示順１
		if (param.getDspOrderDate().equals(InstockReceiveParameter.DISPLAY_ORDER_INSTOCK_DAY))
		// 入荷受付日順
		{
			...
			param.getDspOrderDate().equals(
				InstockReceiveParameter.DISPLAY_ORDER_INSTOCK_PLAN_DAY)) //入荷予定日順
		{
		...
		if (param.getDspOrderItem().equals(InstockReceiveParameter.DISPLAY_ORDER_TICKET)) //伝票No順
		{
			...
			param.getDspOrderItem().equals(InstockReceiveParameter.DISPLAY_ORDER_ITEM)) //商品コード順
		{
		...
		for (int i = 0; i < resultEntity.length; i++)
		{
					...
					< Long.parseLong(getDateValue(resultEntity[i].getRegistDate()))))
			{
				...
				// 荷主コード
				dispData[0].setConsignorCode(resultEntity[i].getConsignorCode());
				// 荷主名
				dispData[0].setConsignorName(resultEntity[i].getConsignorName());
			}
			...
			//予定ケース数
			dispData[i].setPlanCaseQty(
				DisplayUtil.getCaseQty(
					resultEntity[i].getPlanEnableQty(),
					resultEntity[i].getEnteringQty(),
					resultEntity[i].getWorkFormFlag()));
			//予定ピース数
			dispData[i].setPlanPieceQty(
				DisplayUtil.getPieceQty(
					resultEntity[i].getPlanEnableQty(),
					resultEntity[i].getEnteringQty(),
					resultEntity[i].getWorkFormFlag()));
			//実績ケース数
			dispData[i].setResultCaseQty(
				DisplayUtil.getCaseQty(
					resultEntity[i].getResultQty(),
					resultEntity[i].getEnteringQty(),
					resultEntity[i].getWorkFormFlag()));

			//実績ピース数
			dispData[i].setResultPieceQty(
				DisplayUtil.getPieceQty(
					resultEntity[i].getResultQty(),
					resultEntity[i].getEnteringQty(),
					resultEntity[i].getWorkFormFlag()));
			...
			//欠品ケース数
			dispData[i].setShortageCaseQty(
				DisplayUtil.getCaseQty(
					resultEntity[i].getShortageCnt(),
					resultEntity[i].getEnteringQty(),
					resultEntity[i].getWorkFormFlag()));
			//欠品ピース数
			dispData[i].setShortagePieceQty(
				DisplayUtil.getPieceQty(
					resultEntity[i].getShortageCnt(),
					resultEntity[i].getEnteringQty(),
					resultEntity[i].getWorkFormFlag()));
