// $Id: TcInstockReceivePlanListBusiness.java,v 1.1.1.1 2006/08/17 09:34:13 mori Exp $
 ...
 */
package jp.co.daifuku.wms.instockreceive.display.web.tcinstockreceiveplanlist;
...
import jp.co.daifuku.wms.instockreceive.schedule.TcInstockReceivePlanListSCH;

/**
 * Designer : T.Yoshino<BR>
 * Maker : S.Ishigane<BR>
 * <BR>
 * TC入荷予定リスト発行を行う画面クラスです。<BR>
 * TC入荷予定リスト発行を行うスケジュールにパラメータを引き渡します。<BR>
 * <BR>
 * <DIR>
 ...
 * @author  $Author: mori $
 */
public class TcInstockReceivePlanListBusiness extends TcInstockReceivePlanList implements WMSConstants
{
	// Class fields --------------------------------------------------
	// 遷移先アドレス
	// 荷主検索ポップアップアドレス
	...
	// ＴＣ入荷予定一覧アドレス
	private static final String DO_SRCH_TCINSTOCK_LIST = "/instockreceive/listbox/listtcinstockreceiveplanlist/ListTcInstockReceivePlanList.do";
	...
	public void page_ConfirmBack(ActionEvent e) throws Exception
	{
		...
		try
		{
			...
			// スケジュールを開始する
			WmsScheduler schedule = new TcInstockReceivePlanListSCH();
	...
	/** 
	 * 開始入荷日検索ボタンが押下されたときに呼ばれます。<BR>
	 * <BR>
	 * 概要：このメソッドは検索条件をパラメータにセットし、
	 * その検索条件で入荷日検索リストボックスを表示します。<BR>
	 * <BR>
	 * [パラメータ] <BR>
	 *  <DIR>
	 *  荷主コード<BR>
	 ...
	 * <BR>
	 * 概要：入力エリアの入力項目をパラメータにセットし別画面にTC
	 * 入荷予定一覧リストボックスを表示します。<BR>
	 * 本メソッドでは以下の処理を行います。<BR>
	 * <BR>
	 * <DIR>
	 * <BR>
	 * 1.入力項目をパラメータにセットする。<BR>
	 * 2.TC入荷予定一覧リストボックスを表示する。<BR>
	 * </DIR>
	 * 
	 * [パラメータ] <BR>
	 * <DIR>
	...
	public void btn_PDisplay_Click(ActionEvent e) throws Exception
	{
		...
		// 中継画面->結果画面
		redirect(DO_SRCH_TCINSTOCK_LIST, param, DO_SRCH_PROCESS);
	}
