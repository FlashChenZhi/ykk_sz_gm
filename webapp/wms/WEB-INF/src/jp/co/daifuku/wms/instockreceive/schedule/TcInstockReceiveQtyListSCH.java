...
import jp.co.daifuku.wms.instockreceive.report.TcInstockReceiveQtyWriter;

/**
 * Designer : Y.Okamura <BR>
 * Maker : Y.Okamura <BR>
 * <BR>
 * TC入荷実績リスト発行処理を呼び出すためのクラスです。<BR>
 * 画面から入力された内容をパラメータとして受け取り、TC入荷実績リスト発行処理クラスを呼び出します。<BR>
 ...
 * @author  $Author: mori $
 */
public class TcInstockReceiveQtyListSCH extends AbstractInstockReceiveSCH
{
	 ...
	 * このクラスの初期化を行ないます。
	 */
	public TcInstockReceiveQtyListSCH()
	{
		wMessage = null;
	}
	...
	/**
	 * 画面から入力された内容をパラメータとして受け取り、TC入荷実績リストファイル作成処理クラスにパラメータを渡します。<BR>
	...
	public boolean startSCH(Connection conn, Parameter[] startParams) throws ReadWriteException, ScheduleException
	{
		...
		// 印刷クラスを作成します
		TcInstockReceiveQtyWriter writer = createWriter(conn, param);
	 ...
	 * 
	 * @param conn データベースとのコネクションオブジェクト
	 * @param param 検索条件を含むパラメータオブジェクト
	 * @return 印刷処理クラス
	 */
	protected TcInstockReceiveQtyWriter createWriter(Connection conn, InstockReceiveParameter param)
	{
		// 印刷処理クラスに画面で入力された印刷条件をセットする。
		TcInstockReceiveQtyWriter writer = new TcInstockReceiveQtyWriter(conn);
		writer.setConsignorCode(param.getConsignorCode());
		writer.setFromDate(param.getFromInstockReceiveDate());
		writer.setToDate(param.getToInstockReceiveDate());
		writer.setSupplierCode(param.getSupplierCode());
		writer.setCustomerCode(param.getCustomerCode());
		writer.setFromTicketNo(param.getFromTicketNo());
		writer.setToTicketNo(param.getToTicketNo());
