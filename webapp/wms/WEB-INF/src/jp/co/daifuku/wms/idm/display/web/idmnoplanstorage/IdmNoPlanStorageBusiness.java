// $Id: IdmNoPlanStorageBusiness.java,v 1.1.1.1 2006/08/17 09:34:09 mori Exp $
...
import jp.co.daifuku.wms.idm.schedule.IdmNoPlanStorageSCH;
 ...
 * @author  $Author: mori $
 */
public class IdmNoPlanStorageBusiness extends IdmNoPlanStorage implements WMSConstants
{
	// Class fields --------------------------------------------------
	...
	private void setFirstDisp() throws Exception
	{
		...
		try
		{
			...
			WmsScheduler schedule = new IdmNoPlanStorageSCH();
	 ...
	 *   	[返却データ]<BR>
	 *   	<DIR>
	 * 			ためうちエリアの内容を持つ<CODE>IdmControlParameter</CODE>クラスのインスタンスの配列<BR>
	 * 		</DIR>
	 * </DIR>
	 * @param lineno リストセルの行No.
	 * @throws Exception 全ての例外を報告します。 
	 * @return IdmControlParameter[] ためうちエリアの内容を持つ<CODE>IdmControlParameter</CODE>クラスのインスタンスの配列
	 */
	private IdmControlParameter[] setListParam(int lineno) throws Exception
	{
		...
		if (vecParam.size() > 0)
		{
			...
			IdmControlParameter[] listparam = new IdmControlParameter[vecParam.size()];
	...
	public void btn_StorageStart_Click(ActionEvent e) throws Exception
	{
		...
		try
		{
			...
			// スケジュールパラメータへセット
			IdmControlParameter[] param = null;
			// ためうちエリアの全データをセット
