 ...
 * Maker :  <BR>
 * <BR>
 * 荷主一覧リストボックス（マスタパッケージ）用のデータを検索するためのクラスです。<BR>
 * 検索条件をパラメータとして受け取り、荷主一覧の検索を行います。<BR>
 * 尚、このクラスを使用する際はインスタンスをセッションに保持して下さい。
 * 使用後はセッションから削除して下さい。<BR>
 * このクラスでは以下の処理を行います。<BR>
 * <BR>
 * 1.検索処理(<CODE>SessionConsignorRet(Connection conn,	MasterParameter param)</CODE>メソッド)<BR>
 * <DIR>
 *   リストボックス画面が初期表示された時に実行されます。<BR>
 *   <CODE>find(MasterParameter param)</CODE>メソッドを呼び出し出荷予定情報の検索を行います。<BR>
 * <BR>
 *   ＜検索条件＞ 必須項目なし<BR>
 *   <DIR>
 *     荷主コード<BR>
 ...
 * </TABLE>
 * <BR>
 * @version $Revision: 1.2 $, $Date: 2006/11/10 00:34:26 $
 * @author  $Author: suresh $
 */
public class SessionMasterConsignorRet extends SessionRet
{
	// Class fields -----------------------------------------------------
	...
	{
		return ("$Revision: 1.2 $,$Date: 2006/11/10 00:34:26 $");
	}
	 ...
	 */
	public SessionMasterConsignorRet(Connection conn, MasterParameter param) throws Exception
	{
		this.wConn = conn;
	...
	public Parameter[] getEntities()
	{
	    ...
	    if (wLength > 0 && wLength <= DatabaseFinder.MAXDISP)
	    {
	        try
	        {
	            temp = (Consignor[]) ((ConsignorFinder)wFinder).getEntities(wStartpoint, wEndpoint);
	...
	private void find(MasterParameter param) throws Exception
	{
		...
		wFinder = new ConsignorFinder(wConn);
		// カーソルオープン
		wFinder.open();
		count = ((ConsignorFinder)wFinder).search(skey);
		// 初期化
		wLength = count;
		wCurrent = 0;
	}
