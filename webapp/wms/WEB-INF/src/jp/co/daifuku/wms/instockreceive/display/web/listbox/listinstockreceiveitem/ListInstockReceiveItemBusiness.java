// $Id: ListInstockReceiveItemBusiness.java,v 1.1.1.1 2006/08/17 09:34:12 mori Exp $
 ...
 */
package jp.co.daifuku.wms.instockreceive.display.web.listbox.listinstockreceiveitem;
...
import jp.co.daifuku.wms.instockreceive.display.web.listbox.sessionret.SessionInstockReceiveItemPlanRet;
import jp.co.daifuku.wms.instockreceive.display.web.listbox.sessionret.SessionInstockReceiveItemResultRet;
 ...
 * @author  $Author: mori $
 */
public class ListInstockReceiveItemBusiness extends ListInstockReceiveItem implements WMSConstants
{
	// Class fields --------------------------------------------------
	/** 
	 * 商品コードの受け渡しに使用するキーです
	...
	public void page_Load(ActionEvent e) throws Exception
	{
		...
		// 伝票No.
		String ticketnumber =
			request.getParameter(ListInstockReceiveTicketBusiness.TICKETNUMBER_KEY);
		// 商品コード
		...
		// 検索する荷主が予定か実績か判断する
		if (searchitem.equals(InstockReceiveParameter.SEARCHFLAG_PLAN))
		{
			// SessionInstockReceiveItemPlanRet インスタンス生成
			SessionInstockReceiveItemPlanRet listbox =
				new SessionInstockReceiveItemPlanRet(conn, param);
			// Sessionにlistboxを保持
			this.getSession().setAttribute("LISTBOX", listbox);
			setPlanList(listbox, "first");
		}
		else if (searchitem.equals(InstockReceiveParameter.SEARCHFLAG_RESULT))
		{
			// SessionInstockReceiveItemResultRet インスタンス生成
			SessionInstockReceiveItemResultRet listbox =
				new SessionInstockReceiveItemResultRet(conn, param);
			// Sessionにlistboxを保持
			this.getSession().setAttribute("LISTBOX", listbox);
	 ...
	 * @throws Exception 全ての例外を報告します。
	 */
	private void setPlanList(SessionInstockReceiveItemPlanRet listbox, String actionName)
		throws Exception
	{
		// ページ情報をセット
	 ...
	 * @throws Exception 全ての例外を報告します。
	 */
	private void setResultList(SessionInstockReceiveItemResultRet listbox, String actionName)
		throws Exception
	{
		// ページ情報をセット
		listbox.setActionName(actionName);
	...
	public void pgr_D_Next(ActionEvent e) throws Exception
	{
		...
		if (flag.equals(InstockReceiveParameter.SEARCHFLAG_PLAN))
		{
			// Sessionにlistboxを保持
			SessionInstockReceiveItemPlanRet listbox =
				(SessionInstockReceiveItemPlanRet) this.getSession().getAttribute("LISTBOX");
		...
		else if (flag.equals(InstockReceiveParameter.SEARCHFLAG_RESULT))
		{
			// Sessionにlistboxを保持
			SessionInstockReceiveItemResultRet listbox =
				(SessionInstockReceiveItemResultRet) this.getSession().getAttribute("LISTBOX");
