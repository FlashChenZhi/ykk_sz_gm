 ...
 * <BR>
 * <B>1.検索処理(<CODE>SessionInstockReceivePlanListRet(Connection, InstockReceiveParameter)</CODE>メソッド)<BR></B>
 * <DIR>
 ...
 * @@author  $Author: mori $
 */
public class SessionInstockReceivePlanListRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 * @@param param      <code>InstockReceiveParameter</code>検索結果を含むパラメータ
	 * @@throws Exception 何らかの例外が発生した場合に通知されます。
	 */
	public SessionInstockReceivePlanListRet(Connection conn, InstockReceiveParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	 ...
	 * @@param param      <code>InStockRetrievalParameter</code> 検索条件を含むパラメータ
	 */
	...
	{	
		...
		// TC/DC区分
		if(!StringUtil.isBlank(searchParam.getTcdcFlag()))
		{
			if(InstockReceiveParameter.TCDC_FLAG_TC.equals(searchParam.getTcdcFlag()))
			{
				searchKey.setTcdcFlag(InstockReceiveParameter.TCDC_FLAG_TC);
			}
			else if(InstockReceiveParameter.TCDC_FLAG_DC.equals(searchParam.getTcdcFlag()))
			{
				searchKey.setTcdcFlag(InstockReceiveParameter.TCDC_FLAG_DC);
			}
			else if(InstockReceiveParameter.TCDC_FLAG_CROSSTC.equals(searchParam.getTcdcFlag()))
			{
				searchKey.setTcdcFlag(InstockReceiveParameter.TCDC_FLAG_CROSSTC);
			}
			else if(InstockReceiveParameter.TCDC_FLAG_ALL.equals(searchParam.getTcdcFlag()))
			{
				searchKey.setTcdcFlag(InstockReceiveParameter.TCDC_FLAG_TC,"!=");
			}
		}
	...
	private InstockReceiveParameter[] convertToInstockParams(InstockPlan[] inStockPlan)
	{
		...
		for(int i=0;i<inStockPlan.length;++i)
		{
			...
			inStock [i].setTcdcFlag(inStockPlan[i].getTcdcFlag());
			inStock [i].setTcdcName(DisplayUtil.getTcDcValue(inStockPlan[i].getTcdcFlag()));
