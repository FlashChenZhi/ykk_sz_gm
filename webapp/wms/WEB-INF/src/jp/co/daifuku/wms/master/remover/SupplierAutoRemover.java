// $Id: SupplierAutoRemover.java,v 1.1.1.1 2006/08/17 09:34:19 mori Exp $
 ...
 */
public class SupplierAutoRemover
		extends AbstractAutoRemover
{
	...
	//------------------------------------------------------------
	private SupplierHandler p_handler = null ;
	 ...
	 */
	public SupplierAutoRemover(Connection conn)
	...
	protected DatabaseHandler getDBHandler(Connection conn)
	{
		if (p_handler == null)
		{
			p_handler = new SupplierHandler(conn) ;
		}
		return p_handler ;
	}
	 ...
	 * 実績を検索し、最終更新日時で仕入先マスタの使用日時を更新します。
	 * 
			...
			throws ReadWriteException
	{
		...
		// 仕入先コード
		rKey.setSupplierCodeGroup(2);

		// DNRESULTと荷主コード、仕入先コードで結合
		String[] relationalSql = { Result.CONSIGNORCODE.getName(), Result.SUPPLIERCODE.getName() };
		
		// データ検索
		if(rFinder.searchMaster(rKey, Supplier.TABLE_NAME, relationalSql) <= 0)
		{
			return;
		}
		...
		while (rFinder.isNext())
		{
			...
			{
				// 仕入先マスタ更新
				updateUsed(handler, result[i].getConsignorCode(), result[i].getSupplierCode(), result[i].getLastUpdateDate());
			}
		}
	}
	...
	{
		return "$Id: SupplierAutoRemover.java,v 1.1.1.1 2006/08/17 09:34:19 mori Exp $" ;
	}
}
