//$Id: MasterSupplierDataLoader.java,v 1.1.1.1 2006/08/17 09:34:20 mori Exp $
...
* Maker : hota <BR>
* <BR>
* <CODE>MasterSupplierDataLoader</CODE>クラスは、仕入先マスタデータ取込み処理を行うクラスです。<BR>
...
* @author  $Author: mori $
*/
public class MasterSupplierDataLoader extends AbstractExternalDataLoader
{
	// Class fields --------------------------------------------------
	/** 処理結果 (OK) */
	 ...
	 * 処理名
	 */
    protected final String pName = "MasterSupplierDataLoader";
	 ...
	 */
	public MasterSupplierDataLoader()
	{
	}
	...
	public boolean load(Connection conn, WareNaviSystem wns, Parameter searchParam) throws ReadWriteException, ScheduleException
	{
		...
		return createSupplierMaster(conn, wns, searchParam);

	}
	
	// Package methods -----------------------------------------------
	/**
	 * 仕入先マスタファイルのパスとファイル名を取得します。
	 * 
	 * @return String
	...
	protected String GetRetrievalFile() throws ScheduleException
	{
		try
		{
			...
			String DataLoadPath = IO.get("DATALOAD_FOLDER", "MASTER_SUPPLIER");
			String DataLoadFile = IO.get("DATALOAD_FILENAME", "MASTER_SUPPLIER");
	...
	public MasterParameter setParameter(DataLoadCsvReader reader) throws ScheduleException, CsvIllegalDataException
	{
		...
		{
			if (!reader.getAsciiValue("SUPPLIER_CODE").equals(""))
			{
			    mstParam.setSupplierCode(reader.getAsciiValue("SUPPLIER_CODE"));
			}
			else
			{
			    mstParam.setSupplierCode("0");
			}
		}
		else
		{
		...
		{
		    mstParam.setSupplierName(reader.getValue("SUPPLIER_NAME"));
		}
	 ...
	 * @throws ScheduleException 予期しない例外が発生した場合に通知されます。
	 */
	protected boolean createSupplierMaster(Connection conn, WareNaviSystem wns, Parameter searchParam) throws ReadWriteException, ScheduleException
	{
		...
		// データ登録処理
		SupplierOperator supOperator = new SupplierOperator(conn);
		...
		try
		{
			...
			// CSVファイルを読み込みクラス設定
			DRR = new DataLoadCsvReader(DataLoadCsvReader.LOADTYPE_MASTERSUPPLIER, consignorCode, "CONSIGNOR_CODE");
			...
			while (DRR.DataNext())
			{
				...
				ent.setSupplierCode(mstParam.getSupplierCode());
				ent.setSupplierName1(mstParam.getSupplierName());
				...
				iRet = 	supOperator.checkSupplierName(ent);
