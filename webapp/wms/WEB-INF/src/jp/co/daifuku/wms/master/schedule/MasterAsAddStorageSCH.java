//$Id: MasterAsAddStorageSCH.java,v 1.1.1.1 2006/08/17 09:34:20 mori Exp $
 ...
 * <BR>
 * WEBＡＳＲＳ予定外入庫設定（積増）処理を行うためのクラスです。 <BR>
 * 画面から入力された内容をパラメータとして受け取り、ＡＳＲＳ予定外入庫（積増）作業情報の作成処理を行います。 <BR>
 ...
 * </DIR>
 * <BR>
 * 2.次へボタン押下処理1（<CODE>nextCheck()</CODE>メソッド）<BR>
 * <BR>
 * <DIR>
 *	画面から入力された内容をパラメータとして受け取り、作業者コード・パスワードのチェック、該当データの存在チェックの結果を返します。 <BR>
 *	入力された棚Noに在庫情報が存在し、作業者コード、パスワードの内容が正しい場合はtrueを返します。<BR>
 *	入力された棚Noに在庫情報が存在しない場合、エラーメッセージ「指定された棚は、空棚です」をセットし、falseを返します。<BR>
 ...
 * </DIR>
 * <BR>
 * 3.次へボタン押下処理2（<CODE>query()</CODE>メソッド）<BR>
 * <BR>
 * <DIR>
 *	画面から入力された内容をパラメータとして受け取り、条件チェックを行います。 <BR>
 ...
 *	</DIR>
 *   -棚状態チェック処理-<BR>
 *     入力された棚Noにて、在庫情報が存在する事。 <BR>
 *     棚状態が、実棚であること。（他の作業にて引当等されていないこと） <BR>
 *   <BR>
 * </DIR>
 * <BR>
 * 4.入力ボタン押下処理（<CODE>check()</CODE>メソッド）<BR>
 * <BR>
 * <DIR>
 ...
 * </DIR>
 * <BR>
 * 5.入庫開始ボタン押下処理（<CODE>startSCH()</CODE>メソッド） <BR><BR>
 * <BR>
 * <DIR>
 *   ためうちエリアに表示されている内容をパラメータとして受け取り、ＡＳＲＳ予定外入庫設定（積増）処理を行います。 <BR>
 ...
 *   <DIR>
 *     入力された棚Noにて、積増入庫の作業情報の登録処理を行います。 <BR>
 *   </DIR>  
 *   <BR>
 ...
 * @author  $Author: mori $
 */
public class MasterAsAddStorageSCH extends jp.co.daifuku.wms.asrs.schedule.AsAddStorageSCH
{
	// Class variables -----------------------------------------------
	/**
	 * クラス名(予定外入庫設定（積増し）)
	 */
	public static String CLASS_ADDSTORAGE = "MasterAsAddStorageSCH";
	/**
	 * 予定データ補完ラッパクラス
	 */
	 ...
	 */
	public MasterAsAddStorageSCH(Connection conn) throws ReadWriteException, ScheduleException
	{
		wMessage = null;
		...
		throws ScheduleException, ReadWriteException
	{
		...
		wItemOperator = new ItemOperator(conn);
		// 入力エリアの内容
		MasterAsScheduleParameter wParam = (MasterAsScheduleParameter) checkParam;	    
	...
	/**
	 * 画面から入力された内容をパラメータとして受け取り、予定外入庫設定のスケジュールを開始します。<BR>
