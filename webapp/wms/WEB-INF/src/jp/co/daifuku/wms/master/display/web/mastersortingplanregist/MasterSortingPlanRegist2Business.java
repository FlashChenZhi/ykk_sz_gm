// $Id: MasterSortingPlanRegist2Business.java,v 1.1.1.1 2006/08/17 09:34:18 mori Exp $
 ...
 * @author  $Author: mori $
 */
public class MasterSortingPlanRegist2Business extends MasterSortingPlanRegist2 implements WMSConstants
{
	...
	// 仕入先検索ポップアップアドレス
	private static final String DO_SRCH_SUPPLIER = "/master/listbox/listmastersupplier/ListMasterSupplier.do";
	// 仕分場所検索ポップアップアドレス
	...
	{
		// タイトル
		lbl_SettingName.setText(DisplayText.getText(this.getViewState().getString(MasterSortingPlanRegistBusiness.TITLE)));
		...
		// ケース入数
		lbl_JavaSetCaseEntering.setText(WmsFormatter.getNumFormat(this.getViewState().getInt(MasterSortingPlanRegistBusiness.CASEQTY)));
		// ボール入数
		lbl_JavaSetBundleEntering.setText(WmsFormatter.getNumFormat(this.getViewState().getInt(MasterSortingPlanRegistBusiness.BUNDLEQTY)));
		// ケースITF
		lbl_JavaSetCaseItf.setText(this.getViewState().getString(MasterSortingPlanRegistBusiness.CASEITF));
		// ボールITF
		lbl_JavaSetBundleItf.setText(this.getViewState().getString(MasterSortingPlanRegistBusiness.BUNDLEITF));
		
		// １画面目で決定した読取専用かどうかの判断を引き継ぎ、２画面目も表示する
		if (this.getViewState().getBoolean(MasterSortingPlanRegistBusiness.IS_READ_ONLY_KEY))
		{
	 ...
	 * @throws Exception 全ての例外を報告します。 
	 */
	private MasterSortingParameter[] setListParam(int lineno) throws Exception
	{
		...
		for (int i = 1; i < lst_SortingPlanRegist.getMaxRows(); i++)
		{
			...
			// 入力エリア情報
			// 作業者コード
			param.setWorkerCode(this.getViewState().getString(MasterSortingPlanRegistBusiness.WORKERCODE));
			// パスワード
			param.setPassword(this.getViewState().getString(MasterSortingPlanRegistBusiness.PASSWORD));
			// 荷主コード
			...
			// ケース入数
			param.setEnteringQty(Integer.parseInt(this.getViewState().getString(MasterSortingPlanRegistBusiness.CASEQTY)));
			// ボール入数
			param.setBundleEnteringQty(Integer.parseInt(this.getViewState().getString(MasterSortingPlanRegistBusiness.BUNDLEQTY)));
			// ケースITF
			param.setITF(this.getViewState().getString(MasterSortingPlanRegistBusiness.CASEITF));
			// ボールITF
			param.setBundleITF(this.getViewState().getString(MasterSortingPlanRegistBusiness.BUNDLEITF));
			// 端末No
			param.setTerminalNumber(termNo);
			// 荷主コード最終使用日
			param.setConsignorLastUpdateDate(WmsFormatter.getTimeStampDate(
					this.getViewState().getString(MasterSortingPlanRegistBusiness.CONSIG_DATE_KEY)));
			// 商品コード最終使用日
			param.setItemLastUpdateDate(WmsFormatter.getTimeStampDate(
					this.getViewState().getString(MasterSortingPlanRegistBusiness.ITEM_DATE_KEY)));
			...
			// 出荷先コード最終使用日
			param.setCustomerLastUpdateDate(WmsFormatter.getTimeStampDate(CollectionUtils.getString(0, lst_SortingPlanRegist.getValue(LST_HDN))));
			// 仕入先コード最終使用日
			param.setSupplierLastUpdateDate(WmsFormatter.getTimeStampDate(CollectionUtils.getString(1, lst_SortingPlanRegist.getValue(LST_HDN))));
		...
		if (vecParam.size() > 0)
		{
			...
			MasterSortingParameter[] listparam = new MasterSortingParameter[vecParam.size()];
	 ...
	 * @throws Exception 全ての例外を報告します。 
	 */
	private void setListRow(MasterSortingParameter mergeParam) throws Exception
	{
		//ToolTip用のデータを編集
		...
		hiddenList.add(0, WmsFormatter.getTimeStampString(mergeParam.getCustomerLastUpdateDate()));
		hiddenList.add(1, WmsFormatter.getTimeStampString(mergeParam.getSupplierLastUpdateDate()));
		lst_SortingPlanRegist.setValue(LST_HDN, CollectionUtils.getConnectedString(hiddenList));
	...
	{
		this.getViewState().setString(MasterSortingPlanRegistBusiness.MESSAGE, "");
		// 詳細情報登録画面->基本情報設定画面
		forward(MasterSortingPlanRegistBusiness.DO_REGIST);
	}
	...
	public void btn_Input_Click(ActionEvent e) throws Exception
	{
		...
		try
		{
			...
			mergeParam.setConsignorCode(lbl_JavaSetConsignorCode.getText());
			mergeParam.setCustomerCode(txt_CustomerCode.getText());
			mergeParam.setItemCode(lbl_JavaSetItemCode.getText());
			mergeParam.setSupplierCode(txt_SupplierCode.getText());
			mergeParam.setSupplierName(txt_SupplierName.getText());
			...
			txt_SupplierName.setText(mergeParam.getSupplierName());
			
			// スケジュールパラメータへセットします
			MasterSortingParameter param = new MasterSortingParameter();
			// 荷主と商品の最終使用日時をチェックするためセットする
			param.setConsignorLastUpdateDate(WmsFormatter.getTimeStampDate(
					this.getViewState().getString(MasterSortingPlanRegistBusiness.CONSIG_DATE_KEY)));
			param.setItemLastUpdateDate(WmsFormatter.getTimeStampDate(
					this.getViewState().getString(MasterSortingPlanRegistBusiness.ITEM_DATE_KEY)));
			...
			if (schedule.check(conn, param, listparam))
			{
				...
				param.setSupplierLastUpdateDate(mergeParam.getSupplierLastUpdateDate());
	...
	public void btn_Submit_Click(ActionEvent e) throws Exception
	{
		...
		try
		{
			...
			// 更新データ格納用
			MasterSortingParameter[] param = null;
			// ためうちエリアの全データをセットします
