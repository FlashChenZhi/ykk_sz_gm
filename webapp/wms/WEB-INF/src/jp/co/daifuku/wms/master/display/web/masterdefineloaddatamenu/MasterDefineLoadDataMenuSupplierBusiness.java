// $Id: MasterDefineLoadDataMenuSupplierBusiness.java,v 1.1.1.1 2006/08/17 09:34:17 mori Exp $
 ...
 * Maker : hota <BR>
 * <BR>
 * マスタデータ取り込み項目設定(仕入先マスタ項目設定)画面クラスです。 <BR>
 * 入力エリアから入力した内容をパラメータにセットしスケジュールに渡し、仕入先マスタ項目設定を登録します。 <BR>
 ...
 * @author  $Author: mori $
 */
public class MasterDefineLoadDataMenuSupplierBusiness extends MasterDefineLoadDataMenuSupplier implements WMSConstants
{
	// Class fields --------------------------------------------------
	...
	private void dataLoad() throws Exception
	{
		...
		try
		{
			...
			initParam.setSelectDefineLoadData(MasterDefineDataParameter.SELECTDEFINELOADDATA_SUPPLIER);
			...
			if (param != null)
			{
				...
				// 仕入先コード使用桁
				txt_SupplierCodeLen.setText(param[0].getFigure_SupplierCode());
				// 仕入先コード最大桁
				lbl_MaxLenSUpplierCode.setText(param[0].getMaxFigure_SupplierCode());
				// 仕入先コード位置
				txt_SupplierCdPst.setText(param[0].getPosition_SupplierCode());
				
				// 仕入先名称有効
				chk_SupplierName.setChecked(param[0].getValid_SupplierName());
				// 仕入先名称使用桁
				txt_SupplierNameLen.setText(param[0].getFigure_SupplierName());
				// 仕入先名称最大桁
				lbl_MaxLenSupplierName.setText(param[0].getMaxFigure_SupplierName());
				// 仕入先名称位置
				txt_SupplierNamePst.setText(param[0].getPosition_SupplierName());
	...
	{		
		...
		if (!chk_CnsgnrCd.getChecked() && 
				// 仕入先コード
				!chk_SupplierCode.getChecked() &&
				// 仕入先名称
				!chk_SupplierName.getChecked())
		{
			return false;
		}
	...
	public void btn_Submit_Click(ActionEvent e) throws Exception
	{
		...
		WmsCheckker.checkDefine(lbl_SupplierCode, chk_SupplierCode, txt_SupplierCodeLen, lbl_MaxLenSUpplierCode, txt_SupplierCdPst);
		WmsCheckker.checkDefine(lbl_SupplierName, chk_SupplierName, txt_SupplierNameLen, lbl_MaxLenSupplierName, txt_SupplierNamePst);
		...
		try
		{
			...
			// パラメータに値をセットします
			param[0].setSelectDefineLoadData(MasterDefineDataParameter.SELECTDEFINELOADDATA_SUPPLIER);
			...
			// 仕入先コード有効
			param[0].setValid_SupplierCode(chk_SupplierCode.getChecked());
			// 仕入先コード用桁
			param[0].setFigure_SupplierCode(txt_SupplierCodeLen.getText());
			// 仕入先コード最大桁
			param[0].setMaxFigure_SupplierCode(lbl_MaxLenSUpplierCode.getText());
			// 仕入先コード位置
			param[0].setPosition_SupplierCode(txt_SupplierCdPst.getText());
			
			param[0].setValid_SupplierName(chk_SupplierName.getChecked());
			// 仕入先名称使用桁
			param[0].setFigure_SupplierName(txt_SupplierNameLen.getText());
			// 仕入先名称最大桁
			param[0].setMaxFigure_SupplierName(lbl_MaxLenSupplierName.getText());
			// 仕入先名称位置
			param[0].setPosition_SupplierName(txt_SupplierNamePst.getText());
