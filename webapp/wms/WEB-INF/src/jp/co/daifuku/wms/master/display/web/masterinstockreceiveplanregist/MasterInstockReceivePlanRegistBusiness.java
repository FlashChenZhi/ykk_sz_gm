// $Id: MasterInstockReceivePlanRegistBusiness.java,v 1.1.1.1 2006/08/17 09:34:17 mori Exp $
 ...
 * @author  $Author: mori $
 */
public class MasterInstockReceivePlanRegistBusiness extends MasterInstockReceivePlanRegist implements WMSConstants
{
	// 遷移先アドレス
	// 荷主検索ポップアップアドレス
	...
	protected static final String DO_REGIST = "/master/masterinstockreceiveplanregist/MasterInstockReceivePlanRegist.do";
	// 入荷予定登録2(詳細情報設定画面)アドレス
	protected static final String DO_REGIST2 = "/master/masterinstockreceiveplanregist/MasterInstockReceivePlanRegist2.do";
	...
	/**
	 * 仕入先コードの最終使用日を保持するViewState用キーです
	 */
	public static final String SUPPL_DATE_KEY = "SUPPL_DATE";
	...
	public void page_Load(ActionEvent e) throws Exception
	{
		...
		// ＴＣ／ＤＣ区分
		if (!StringUtil.isBlank(this.getViewState().getString(MasterInstockReceivePlanRegistBusiness.TCDCFLG)))
		{
			String str = this.getViewState().getString(MasterInstockReceivePlanRegistBusiness.TCDCFLG);
			...
			else if (str.equals( InstockReceiveParameter.TCDC_FLAG_TC ))
			{
				...
				if (!this.getViewState().getBoolean(IS_READ_ONLY_KEY))
				{
					// 出荷先名称
	...
	private void setConsignor() throws Exception
	{
		...
		try
		{
			...
			param = (MasterInstockReceiveParameter) schedule.initFind(conn, param);
			...
			if (param != null)
			{
				...
				if (param.getMergeType() == MasterInstockReceiveParameter.MERGE_TYPE_OVERWRITE)
				{
	...
	public void btn_Next_Click(ActionEvent e) throws Exception
	{
		...
		try
		{
			...
			this.getViewState().setString(SUPPL_DATE_KEY, WmsFormatter.getTimeStampString(mergeParam.getSupplierLastUpdateDate()));
