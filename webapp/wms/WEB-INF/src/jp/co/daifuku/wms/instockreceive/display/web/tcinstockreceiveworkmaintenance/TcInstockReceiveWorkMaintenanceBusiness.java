// $Id: TcInstockReceiveWorkMaintenanceBusiness.java,v 1.2 2006/11/10 00:32:58 suresh Exp $
 ...
 */
package jp.co.daifuku.wms.instockreceive.display.web.tcinstockreceiveworkmaintenance;
...
import jp.co.daifuku.wms.instockreceive.schedule.TcInstockReceiveWorkMaintenanceSCH;
;
 ...
 * Maker : S.Yoshida <BR>
 * <BR>
 * TC入荷作業メンテナンス画面クラスです。<BR>
 * TC入荷作業メンテナンス処理を行うスケジュールにパラメータを引き渡します。<BR>
 ...
 * </TABLE>
 * <BR>
 * @version $Revision: 1.2 $, $Date: 2006/11/10 00:32:58 $
 * @author  $Author: suresh $
 */
public class TcInstockReceiveWorkMaintenanceBusiness extends TcInstockReceiveWorkMaintenance implements WMSConstants
{
	// Class fields --------------------------------------------------
	/**
	 * ViewState用：作業者コード
	...
	protected boolean checkContainNgText(int rowNo)
	{
		...
		lst_STcInstkRcvWkMtn.setCurrentRow(rowNo);

		if(!checker.checkContainNgText(
				lst_STcInstkRcvWkMtn.getValue(8) ,
				rowNo,
				lst_STcInstkRcvWkMtn.getListCellColumn(8).getResourceKey() )
				)
		{
	...
	public void btn_PSearchCustomerCode_Click(ActionEvent e) throws Exception
	{
		...
		if (pul_WorkStatus.getSelectedIndex() != 0)
		{
			...
			if (pul_WorkStatus.getSelectedIndex() == 1)
			{
				...
				wStatus[1] = new String(InstockReceiveParameter.STATUS_FLAG_PARTIAL_COMPLETION);
				forwardParam.setParameter(
				ListInstockReceiveCustomerBusiness.WORKSTATUSSUPPLIER_KEY,
					wStatus);
			}
			else if (pul_WorkStatus.getSelectedIndex() == 2)
			{
	...
	public void btn_View_Click(ActionEvent e) throws Exception
	{
		...
		// TC入荷作業メンテナンススケジュールのインスタンス生成
		WmsScheduler schedule = new TcInstockReceiveWorkMaintenanceSCH();
		// 入荷パラメータ配列
	 ...
	 * @throws Exception 
	 */
	public void lst_STcInstkRcvWkMtn_EnterKey(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_STcInstkRcvWkMtn_TabKey(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_STcInstkRcvWkMtn_InputComplete(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_STcInstkRcvWkMtn_ColumClick(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_STcInstkRcvWkMtn_Server(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_STcInstkRcvWkMtn_Change(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_STcInstkRcvWkMtn_Click(ActionEvent e) throws Exception
	{
	}
	...
	private void addList(InstockReceiveParameter[] param) throws Exception
	{
		...
		// 入荷ケース数
		String label_caseqty = DisplayText.getText("LBL-W0081");
		// 入荷ピース数
		String label_pieceqty = DisplayText.getText("LBL-W0082");
		// 状態
		...
		{
			// リストセルに行を追加する。
			lst_STcInstkRcvWkMtn.addRow();
			// 値をセットする行を選択する。
			lst_STcInstkRcvWkMtn.setCurrentRow(lst_STcInstkRcvWkMtn.getMaxRows() - 1);
			
			// 検索結果を各セルにセットしていく
			// HIDDEN項目
			lst_STcInstkRcvWkMtn.setValue(0, createHiddenList(viewParam[i]));
			// 仕入先コード
			lst_STcInstkRcvWkMtn.setValue(1, viewParam[i].getSupplierCode());
			// 出荷先コード
			lst_STcInstkRcvWkMtn.setValue(2, viewParam[i].getCustomerCode());
			// 商品コード
			lst_STcInstkRcvWkMtn.setValue(3, viewParam[i].getItemCode());
			// ケース入数
			lst_STcInstkRcvWkMtn.setValue(4, Formatter.getNumFormat(viewParam[i].getEnteringQty()));
			// 作業予定ケース数
			lst_STcInstkRcvWkMtn.setValue(5, Formatter.getNumFormat(viewParam[i].getPlanCaseQty()));
			// 入荷ケース数
			lst_STcInstkRcvWkMtn.setValue(6, Formatter.getNumFormat(viewParam[i].getResultCaseQty()));
			// 状態
			if (viewParam[i].getStatusFlagL().equals(InstockReceiveParameter.STATUS_FLAG_UNSTARTED))
			{
				// 未開始
				lst_STcInstkRcvWkMtn.setValue(7, L_STATUSFLAGUNSTARTED);
			}
			else if (viewParam[i].getStatusFlagL().equals(InstockReceiveParameter.STATUS_FLAG_WORKING))
			{
				// 作業中
				lst_STcInstkRcvWkMtn.setValue(7, L_STATUSFLAGWORKING);
			}
			else if (viewParam[i].getStatusFlagL().equals(InstockReceiveParameter.STATUS_FLAG_COMPLETION))
			{
				// 完了
				lst_STcInstkRcvWkMtn.setValue(7, L_STATUSFLAGCOMPLETION);
			}
			// 賞味期限
			lst_STcInstkRcvWkMtn.setValue(8, viewParam[i].getUseByDate());
			// 実績報告
			lst_STcInstkRcvWkMtn.setValue(9, viewParam[i].getReportFlagName());
			// 伝票No.
			lst_STcInstkRcvWkMtn.setValue(10, viewParam[i].getInstockTicketNo());
			// 仕入先名称
			lst_STcInstkRcvWkMtn.setValue(11, viewParam[i].getSupplierName());
			// 出荷先名称
			lst_STcInstkRcvWkMtn.setValue(12, viewParam[i].getCustomerName());
			// 商品名称
			lst_STcInstkRcvWkMtn.setValue(13, viewParam[i].getItemName());
			// ボール入数
			lst_STcInstkRcvWkMtn.setValue(14, Formatter.getNumFormat(viewParam[i].getBundleEnteringQty()));
			// 作業予定ピース数
			lst_STcInstkRcvWkMtn.setValue(15, Formatter.getNumFormat(viewParam[i].getPlanPieceQty()));
			// 入荷ピース数
			lst_STcInstkRcvWkMtn.setValue(16, Formatter.getNumFormat(viewParam[i].getResultPieceQty()));
			// 行No.
			lst_STcInstkRcvWkMtn.setValue(17, String.valueOf(viewParam[i].getInstockLineNo()));
			...
			// 出荷先名称
			toolTip.add(label_customername, viewParam[i].getCustomerCode());
			// 商品名称
			toolTip.add(label_itemname, viewParam[i].getItemName());
			// 入荷ケース数
			toolTip.add(label_caseqty, Formatter.getNumFormat(viewParam[i].getResultCaseQty()));
			// 入荷ピース数
			toolTip.add(label_pieceqty, Formatter.getNumFormat(viewParam[i].getResultPieceQty()));
			// 状態
			...
			lst_STcInstkRcvWkMtn.setToolTip(lst_STcInstkRcvWkMtn.getCurrentRow(), toolTip.getText());
		}
	...
	private void updateList(String pButtonType) throws Exception
	{
		...
		Connection wConn = null;
		// Vectorインスタンス生成
		Vector vecParam = new Vector(lst_STcInstkRcvWkMtn.getMaxRows());
		// 入荷パラメータ配列
		...
		try
		{
			...
			for (int i = 1; i < lst_STcInstkRcvWkMtn.getMaxRows(); i++)
			{
				// 作業行を選択する
				lst_STcInstkRcvWkMtn.setCurrentRow(i);
				
				// ためうちの入力チェックを行う。
				// 入荷ケース数
				lst_STcInstkRcvWkMtn.validate(6, false);
				// 状態
				lst_STcInstkRcvWkMtn.validate(7, true);
				// 賞味期限
				lst_STcInstkRcvWkMtn.validate(8, false);
				// 入荷ピース数
				lst_STcInstkRcvWkMtn.validate(16, false);
				...
				if (isChangeData(pButtonType))
				{
					...
					// 出荷先コード
					wParam.setCustomerCode(this.getViewState().getString(CUSTOMERCODE));
					// 開始伝票No.
					...
					// 更新値をセットする。
					// 入荷ケース数	
					wParam.setResultCaseQty(Formatter.getInt(lst_STcInstkRcvWkMtn.getValue(6)));
					// 入荷ピース数
					wParam.setResultPieceQty(Formatter.getInt(lst_STcInstkRcvWkMtn.getValue(16)));
					// 状態
					if(lst_STcInstkRcvWkMtn.getValue(7).equals(L_STATUSFLAGUNSTARTED))
					{
						// 未開始
						wParam.setStatusFlagL(InstockReceiveParameter.STATUS_FLAG_UNSTARTED);
					}
					else if(lst_STcInstkRcvWkMtn.getValue(7).equals(L_STATUSFLAGWORKING))
					{
						// 作業中
						wParam.setStatusFlagL(InstockReceiveParameter.STATUS_FLAG_WORKING);
					}
					else if(lst_STcInstkRcvWkMtn.getValue(7).equals(L_STATUSFLAGCOMPLETION))
					{
						// 完了
						wParam.setStatusFlagL(InstockReceiveParameter.STATUS_FLAG_COMPLETION);
					}
					// 賞味期限
					wParam.setUseByDate(lst_STcInstkRcvWkMtn.getValue(8));
					// ケース入数
					wParam.setEnteringQty(Formatter.getInt(lst_STcInstkRcvWkMtn.getValue(4)));
					// 押下ボタン
					wParam.setButtonType(pButtonType);
					// ためうち行
					wParam.setRowNo(lst_STcInstkRcvWkMtn.getCurrentRow());
					// 端末No.
					UserInfoHandler userHandler = new UserInfoHandler((DfkUserInfo) this.getUserInfo());
					wParam.setTerminalNumber(userHandler.getTerminalNo());
					// 作業No.
					wParam.setJobNo(CollectionUtils.getString(HIDDEN4, lst_STcInstkRcvWkMtn.getValue(0)));
					// 最終更新日時
					wParam.setLastUpdateDate(WmsFormatter.getTimeStampDate(CollectionUtils.getString(HIDDEN5, lst_STcInstkRcvWkMtn.getValue(0))));
	...
	private boolean isChangeData(String pButtonType) throws Exception
	{
		...
		if (pButtonType.equals(InstockReceiveParameter.BUTTON_MODIFYSUBMIT))
		{
			// HIDDEN項目を取得する。
			// 入荷ケース数
			String wOrgCaseQty = CollectionUtils.getString(HIDDEN0, lst_STcInstkRcvWkMtn.getValue(0));
			// 入荷ピース数
			String wOrgPieceQty = CollectionUtils.getString(HIDDEN1, lst_STcInstkRcvWkMtn.getValue(0));
			// 状態
			String wOrgStatus = CollectionUtils.getString(HIDDEN2, lst_STcInstkRcvWkMtn.getValue(0));
			// 賞味期限
			String wOrgUseByDate = CollectionUtils.getString(HIDDEN3, lst_STcInstkRcvWkMtn.getValue(0));
			
			// 入力テキストがひとつも変更されていない場合処理を行わない。
			// 入荷ケース数
			if (!lst_STcInstkRcvWkMtn.getValue(6).equals(wOrgCaseQty))
			{
				return true;
			}
			// 状態
			else if (!lst_STcInstkRcvWkMtn.getValue(7).equals(wOrgStatus))
			{
				return true;
			}
			// 賞味期限
			// 状態が完了以外は変更とみなさない
			 ...
			 && !lst_STcInstkRcvWkMtn.getValue(8).equals(wOrgUseByDate))
			{
				return true;
			}
			// 入荷ピース数
			else if (!lst_STcInstkRcvWkMtn.getValue(16).equals(wOrgPieceQty))
			{
				return true;
			}
			return false;
