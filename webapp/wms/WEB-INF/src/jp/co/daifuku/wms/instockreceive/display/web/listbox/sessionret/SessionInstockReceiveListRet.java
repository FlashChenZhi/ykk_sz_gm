// $Id: SessionInstockReceiveListRet.java,v 1.1.1.1 2006/08/17 09:34:13 mori Exp $
 ...
 * <BR>
 * <B>1.検索処理(<CODE>SessionInstockReceiveListRet(Connection, InstockReceiveParameter)</CODE>メソッド)<BR></B>
 * <DIR>
 ...
 *     <tr><td>*</td><td>クロス/DC区分</td><td>：</td><td>TcDcFlag</td></tr>
 ...
 *     <tr><td></td><td>ｸﾛｽ/DC</td><td>：</td><td>TcdcName</td></tr>
 *   </table>
 *   </DIR>
 * </DIR>
 * 
 ...
 * @author  $Author: mori $
 */
public class SessionInstockReceiveListRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionInstockReceiveListRet(Connection conn, InstockReceiveParameter param) throws Exception
	{
		this.wConn = conn;
	...
	private void find(InstockReceiveParameter param) throws Exception
	{
		...
		if (!StringUtil.isBlank(param.getTcdcFlag()))
		{
			...
			{
				// クロス
				viewKey.setTcdcFlag(WorkingInformation.TCDC_FLAG_CROSSTC);
			...
			{
				// DC
				viewKey.setTcdcFlag(WorkingInformation.TCDC_FLAG_DC);
				consignorkey.setTcdcFlag(WorkingInformation.TCDC_FLAG_DC);
			}
			else if (param.getTcdcFlag().equals(InstockReceiveParameter.TCDC_FLAG_ALL))
			{
				// 全て
				// 全てが選択された場合はTC以外を選択する
		...
		{   // 表示順が伝票No.順の時
			if(param.getDspOrder().equals(InstockReceiveParameter.DISPLAY_ORDER_TICKET))
			{
				...
				// 賞味期限
				viewKey.setResultUseByDateOrder(6,true);
				// クロス／ＤＣ
				viewKey.setTcdcFlagOrder(7,false);
			}
			// 表示順が商品コード順の時
	...
	private InstockReceiveParameter[] getDispData(WorkingInformation[] work)
	{
		...
		for (int i = 0; i < work.length; i++)
		{
			...
			// クロス／ＤＣ
			tempParam.setTcdcName(DisplayUtil.getTcDcValue(work[i].getTcdcFlag()));
