...
<xsl:template match="/template">
  ...
  <!-- default value -->
  <xsl:variable name="def_val">ListBox</xsl:variable>

  <!-- otput "SELECT" tag -->
  <select>

    <!-- set attribute "multiple" -->
    <xsl:if test="property[@name='multiple']/@value='true'">
    ...
    <xsl:if test="count(list-items[@type='ListBoxItems']/list-item[@type='ListBoxItem'])=0">
    ...
    <!-- output "OPTION" tags -->
    <xsl:for-each select="list-items[@type='ListBoxItems']/list-item[@type='ListBoxItem']">
