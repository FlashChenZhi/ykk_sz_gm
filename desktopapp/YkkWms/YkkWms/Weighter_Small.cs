...
namespace YkkWms
{
    class Weighter_Small
    {
        private static Weighter_Small weighter = null;
        private static int latency = int.Parse(AppConfig.Get("Latency"));
        public static Weighter_Small Instance
        {
            get
            {
                if (weighter == null)
                {
                    weighter = new Weighter_Small();
                }
                return weighter;
            }
        }

        SerialPort sp = new SerialPort(AppConfig.Get("ComPort_Small"));
        private Weighter_Small()
        {
            InitSerialPort();
        }
