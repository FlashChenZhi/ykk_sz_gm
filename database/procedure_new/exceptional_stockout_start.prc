DROP PROCEDURE AWC.EXCEPTIONAL_STOCKOUT_START;

CREATE OR REPLACE PROCEDURE AWC.exceptional_stockout_start (
...
IS
   ...
   PROCEDURE update_stock_data (
      ...
      in_remark           IN     fnzaiko.remark%TYPE,
      io_return_code      IN OUT NUMBER,
      io_return_message   IN OUT VARCHAR2)
   IS
   BEGIN
      ...
      THEN
         ...
         UPDATE fnzaiko
            SET skanosu = 0, remark = in_remark
          WHERE systemid = in_systemid;
      --      ELSE
      --         UPDATE fnzaiko
