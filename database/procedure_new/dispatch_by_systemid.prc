DROP PROCEDURE AWC.DISPATCH_BY_SYSTEMID;

CREATE OR REPLACE PROCEDURE AWC.dispatch_by_systemid (
...
BEGIN
   ...
   wk_priority := ykk_global_defination.trans_priority_normal;

   IF    TRIM (wk_rsvinfo_row.cmt) IS NULL
      OR substr (wk_rsvinfo_row.cmt, 1, 1) != '1'
   THEN
      wk_priority := ykk_global_defination.trans_priority_normal;
   ELSE
      wk_priority := ykk_global_defination.trans_priority_urgency;
   END IF;
