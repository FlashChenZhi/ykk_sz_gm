DROP PROCEDURE AWC.AUTO_DISPATCH_BY_PLANKEY;

CREATE OR REPLACE PROCEDURE AWC.auto_dispatch_by_plankey (
...
IS
   ...
   stock_not_enough_exception      EXCEPTION;
BEGIN
   io_return_code := 0;
   io_return_message := '';
   ...
   IF in_userid = 'auto' AND wk_expected_stockout_num > 0
   THEN
      RAISE stock_not_enough_exception;
   END IF;
  ...
  <<endlabel>>
   ...
   END IF;
EXCEPTION
   WHEN stock_not_enough_exception
   THEN
      IF item_list_el%ISOPEN = TRUE
      THEN
         CLOSE item_list_el;
