DROP PROCEDURE AWC.STOCK_BACK;

CREATE OR REPLACE PROCEDURE AWC.stock_back (
...
IS
   ...
   BEGIN
      ...
      ELSE
         UPDATE fnzaiko
            SET bucket_no = in_bucket_no, bag_flag = in_bag_flag
          WHERE ticket_no = in_ticket_no;
      END IF;
   EXCEPTION
      WHEN OTHERS
      ...
      UPDATE fnlocat
         SET bucket_no = in_bucket_no,
             ...
             ailestno =
                TO_CHAR (
                   TRUNC (
                        (TO_NUMBER (SUBSTR (in_location_no, 2, 3)) + 1) / 2
                      + 9000))
       WHERE systemid = (SELECT systemid
                           FROM fnhanso
                          WHERE mckey = in_mckey);
...
BEGIN
   ...
   SELECT    '9'
          || LPAD (TO_CHAR (TRUNC ( (TO_NUMBER (bankno) + 1) / 2)), 3, '0')
   ...
   IF wk_fngset_row.motostno IN
         ('2206', '2209', '3202', '3205', '1212', '1215', '1218')
   THEN
