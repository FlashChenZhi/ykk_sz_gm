DROP PROCEDURE YKK.STOCK_BACK_EXPORT;

CREATE OR REPLACE PROCEDURE YKK.stock_back_export (
...
BEGIN
   ...
   delete_stock_data_on_export (wk_systemid, io_return_code,
                                io_return_message);
...
EXCEPTION
   ...
   THEN
      ...
      io_return_message := '回库排出模式设定时发生错误' || ' ' || SQLERRM;
      DBMS_OUTPUT.put_line (SQLERRM);
