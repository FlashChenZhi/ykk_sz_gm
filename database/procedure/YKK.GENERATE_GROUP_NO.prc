DROP PROCEDURE YKK.GENERATE_GROUP_NO;

CREATE OR REPLACE PROCEDURE YKK.generate_group_no (
...
BEGIN
   ...
   THEN
      ...
      io_return_message :=
                         '生成GroupNo时发生错误' || ' ' || io_return_message;
   END IF;
EXCEPTION
   WHEN OTHERS
   THEN
      io_return_code := 8000123;
      io_return_message := '生成GroupNo时发生错误' || ' ' || SQLERRM;
