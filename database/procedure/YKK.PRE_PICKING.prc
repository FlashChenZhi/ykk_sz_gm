DROP PROCEDURE YKK.PRE_PICKING;

CREATE OR REPLACE PROCEDURE YKK.pre_picking (
      ...
      SELECT *
       ...
       WHERE mckey = in_mckey
         ...
         AND in_order_no =
                (SELECT order_no
                   FROM fnsyotei
                  WHERE fnsyotei.retrieval_plankey = fnsiji.retrieval_plan_key);
...
BEGIN
   ...
   IF io_picking_type = ykk_global_defination.picking_type_normal
   THEN
      SELECT SUM (nyusyusu)
      ...
      THEN
         ...
         SELECT section, line, retrieval_station, picking_num
           INTO wk_section, wk_line, wk_retrieval_station_temp, wk_picking_num
           FROM (SELECT   section, line, retrieval_station,
      ...
      ELSE
         SELECT COUNT (*) - 1
           INTO io_remaining_job_num
           FROM (SELECT   fnsiji.customer_code, fnsiji.retrieval_station,
                          fnsyotei.order_no
         ...
         SELECT customer_code, retrieval_station, order_no,
                picking_num
           INTO wk_customer_code, wk_retrieval_station_temp, wk_order_no,
                wk_picking_num
           FROM (SELECT   fnsiji.customer_code, fnsiji.retrieval_station,
                          fnsyotei.order_no,
                          SUM (fnsiji.nyusyusu) AS picking_num
