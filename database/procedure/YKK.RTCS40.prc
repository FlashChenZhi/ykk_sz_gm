DROP PROCEDURE YKK.RTCS40;

CREATE OR REPLACE PROCEDURE YKK.RTCS40
		(
...
DEF_MYNAME		CONSTANT VARCHAR2(10)	:= 'RTCS40' ;
...
CURSOR
	...
	WHERE
		...
		(
			...
			FNHANSO.HJYOTAIFLG	=	PA_AWC_DEFINE.HJYOTAIFLG_KAISI		OR
			FNHANSO.HJYOTAIFLG	=	PA_AWC_DEFINE.HJYOTAIFLG_OUTOUMATI	OR
			FNHANSO.HJYOTAIFLG	=	PA_AWC_DEFINE.HJYOTAIFLG_SIJIZUMI
		)																AND
...
BEGIN
	...
	BEGIN
		...
		SET
			FHSTORAGEINFO.PROC_FLAG   =   '0'
 		WHERE
			FHSTORAGEINFO.TICKET_NO   =   D_FNSIJI.TICKET_NO   AND
			FHSTORAGEINFO.PROC_FLAG   =   '1' ;
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			IF PA_AWC_DEFINE.NOTFOUND_NG = SQLCODE THEN IO_RC		:=	FUKKI.RET_DATNOT ; WK_MSGBUFF	:=	'SYSTEMID[' || D_FNHANSO.SYSTEMID || ']'; WK_LOGMSG	:=	SUBSTRB ((MSGCVT (GSC_MSGDEF.GMSG4003, 'FNZAIKO', WK_MSGBUFF)),
	...
	LOOP
		...
		IF FNHANSIJI01_CUR%NOTFOUND  =  TRUE		THEN
			...
			IF FNHANSIJI01_CUR%ROWCOUNT  <=  0	THEN
				...
				GOTO	RTCS40_ENDLABEL ;
			END IF ;




			RTCS40_40 (IO_RC, WK_LOGMSG) ;
		...
		IF D_FNHANSO.SAGYOKBN = PA_AWC_DEFINE.SAGYOKBN_IKISAKISIJI	THEN
			RTCS40_70 (IO_RC, WK_LOGMSG) ;
			IF IO_RC != FUKKI.RET_OK THEN
				GOTO	RTCS40_ENDLABEL ;
			END IF ;
		END IF ;
