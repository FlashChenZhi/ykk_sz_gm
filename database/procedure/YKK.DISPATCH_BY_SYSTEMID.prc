DROP PROCEDURE YKK.DISPATCH_BY_SYSTEMID;

CREATE OR REPLACE PROCEDURE YKK.dispatch_by_systemid (
...
BEGIN
   ...
   INSERT INTO fnhanso
               (hansokey, systemid, mckey, startstno,
                endstno, motostno, sakistno,
                hjyotaiflg,
                nyusyukbn,
                yusenkbn,
                sagyokbn,
                sijisyosai, bucket_no, setkbn
               )
        VALUES (wk_hansokey, in_systemid, wk_mckey, wk_fnlocat_row.ailestno,
                wk_endstno, wk_fnlocat_row.ailestno, wk_next_st,
                ...
                wk_stockout_mode, wk_fnzaiko_row.bucket_no, '2'
               );
   ...
   INSERT INTO fnsiji
               (hansokey, mckey, ticket_no,
                zaikey, color_code,
                section, line, nyusyusu,
                retrieval_plan_key, label_key,
                line_type, depot_code,
                customer_code,
                manage_item_flag,
                retrieval_station
               )
        VALUES (wk_hansokey, wk_mckey, wk_fnzaiko_row.ticket_no,
                wk_fnzaiko_row.zaikey, wk_fnzaiko_row.color_code,
                wk_rsvinfo_row.section, wk_rsvinfo_row.line, in_stockout_num,
                in_retrieval_plan_key, wk_label_key,
                wk_rsvinfo_row.line_type, wk_rsvinfo_row.depot_code,
                wk_rsvinfo_row.customer_code,
                wk_fnzaiko_row.manage_item_flag,
                wk_rsvinfo_row.retrieval_station
               );

   create_label_data_for_stockout
                            (wk_label_key,
                             ykk_global_defination.label_type_normal_stockout,
