DROP PROCEDURE YKK.PICKING_ABNORMAL_START;

CREATE OR REPLACE PROCEDURE YKK.picking_abnormal_start (
...
IS
   ...
   BEGIN
      ...
      LOOP
         ...
         SELECT COUNT (nyusyusu)
           INTO wk_count
           FROM fnsiji
          WHERE mckey = in_mckey
            AND retrieval_plan_key = plankey_list_iterator.retrieval_plan_key;

         UPDATE fnsyotei
            SET retrieval_alloc_qty = retrieval_alloc_qty - wk_count,
                proc_flag =
                          ykk_global_defination.reservation_info_not_processed
          WHERE retrieval_plankey = plankey_list_iterator.retrieval_plan_key;
...
--   END IF;
      ...
      update_rsv_data_on_abnormal (wk_mckey, io_return_code,
                                   io_return_message);
