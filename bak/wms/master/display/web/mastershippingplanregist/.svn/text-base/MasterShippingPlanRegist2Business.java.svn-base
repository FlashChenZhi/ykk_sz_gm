// $Id: MasterShippingPlanRegist2Business.java,v 1.1.1.1 2006/08/17 09:34:18 mori Exp $
 ...
 * @author  $Author: mori $
 */
public class MasterShippingPlanRegist2Business extends MasterShippingPlanRegist2 implements WMSConstants
{
	// Class fields --------------------------------------------------
	...
	public void page_Load(ActionEvent e) throws Exception
	{
		try
		{
			// 画面名をセットする
			// タイトル
			lbl_SettingName.setText(DisplayText.getText(this.getViewState().getString(MasterShippingPlanRegistBusiness.TITLE_KEY)));		
			...
			if (this.getViewState().getBoolean(MasterShippingPlanRegistBusiness.IS_READ_ONLY_KEY))
			{
	 ...
	 * @throws Exception 全ての例外を報告します。 
	 */
	protected MasterShippingParameter[] setListParam(int lineno) throws Exception
	{
		...
		for (int i = 1; i < lst_SShpPlnDaRst.getMaxRows(); i++)
		{
			...
			param.setConsignorLastUpdateDate(WmsFormatter.getTimeStampDate(
					this.getViewState().getString(MasterShippingPlanRegistBusiness.CONSIG_DATE_KEY)));
			// 出荷先コード最終使用日
			param.setCustomerLastUpdateDate(WmsFormatter.getTimeStampDate(
					this.getViewState().getString(MasterShippingPlanRegistBusiness.CUSTOM_DATE_KEY)));
			...
			// TC/DC区分(隠し項目)
			param.setTcdcFlag(CollectionUtils.getString(0, lst_SShpPlnDaRst.getValue(0)));
			...
			// 商品コード最終使用日
			param.setItemLastUpdateDate(WmsFormatter.getTimeStampDate(CollectionUtils.getString(1, lst_SShpPlnDaRst.getValue(0))));
			// 仕入先コード最終使用日
			param.setSupplierLastUpdateDate(WmsFormatter.getTimeStampDate(CollectionUtils.getString(2, lst_SShpPlnDaRst.getValue(0))));
		...
		if (vecParam.size() > 0)
		{
			...
			MasterShippingParameter[] listparam = new MasterShippingParameter[vecParam.size()];
	 ...
	 * @throws Exception 全ての例外を報告します。 
	 */
	protected void setList(MasterShippingParameter mergeParam) throws Exception
	{
		//ToolTip用のデータを編集
		...
		{
			// DC
			hiddenList.add(0, ShippingParameter.TCDC_FLAG_DC);
		}
		else if (rdo_CrossDCFlagCross.getChecked())
		{
			// クロス
			hiddenList.add(0, ShippingParameter.TCDC_FLAG_CROSSTC);
		}
		else if (rdo_CrossDCFlagTC.getChecked())
		{
			// TC
			hiddenList.add(0, ShippingParameter.TCDC_FLAG_TC);
		}
		// 商品コード最終使用日
		hiddenList.add(1, WmsFormatter.getTimeStampString(mergeParam.getItemLastUpdateDate()));
		// 仕入先コード最終使用日
		hiddenList.add(2, WmsFormatter.getTimeStampString(mergeParam.getSupplierLastUpdateDate()));
		lst_SShpPlnDaRst.setValue(0, CollectionUtils.getConnectedString(hiddenList));
	...
	{
		// 詳細情報登録画面->基本情報設定画面
		forward("/master/mastershippingplanregist/MasterShippingPlanRegist.do");
	}
	...
	public void btn_PSearchSpl_Click(ActionEvent e) throws Exception
	{
		...
		// 処理中画面->結果画面
		redirect(
			"/master/listbox/listmastersupplier/ListMasterSupplier.do",
			param,
			"/progress.do");
	}
	...
	public void btn_Submit_Click(ActionEvent e) throws Exception
	{
		...
		try
		{
			...
			// スケジュールパラメータへセット
			MasterShippingParameter[] param = null;
			// ためうちエリアの全データをセット
	...
	public void lst_SShpPlnDaRst_Click(ActionEvent e) throws Exception
	{
		...
		else if (lst_SShpPlnDaRst.getActiveCol() == 2)
		{
			...
			// TC/DC(隠し項目)
			if (CollectionUtils.getString(0, lst_SShpPlnDaRst.getValue(0)).equals(ShippingParameter.TCDC_FLAG_DC))
			{
				// DC
			...
			else if (CollectionUtils.getString(0, lst_SShpPlnDaRst.getValue(0)).equals(ShippingParameter.TCDC_FLAG_CROSSTC))
			{
				// クロス
				rdo_CrossDCFlagDC.setChecked(false);
			...
			else if (CollectionUtils.getString(0, lst_SShpPlnDaRst.getValue(0)).equals(ShippingParameter.TCDC_FLAG_TC))
			{
				// TC
				rdo_CrossDCFlagDC.setChecked(false);
				rdo_CrossDCFlagCross.setChecked(false);
