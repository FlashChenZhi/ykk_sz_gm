// $Id: CorrectSupplierMasterBusiness.java,v 1.1.1.1 2006/08/17 09:34:16 mori Exp $
...
import jp.co.daifuku.wms.master.display.web.listbox.listmastersupplier.ListMasterSupplierBusiness;
 ...
 * @author  $Author: mori $
 */
public class CorrectSupplierMasterBusiness extends CorrectSupplierMaster implements WMSConstants
{
	// 遷移先アドレス
	// 荷主検索ポップアップアドレス
	...
	// 検索条件
	protected static final String searchKey = "1";
	// Class fields --------------------------------------------------
	...
	public void page_Load(ActionEvent e) throws Exception
	{
		...
		// 仕入先コードに入力値があるとき
		if (!StringUtil.isBlank(getViewState().getString("SupplierCode")))
		{
			txt_supplierCode.setText(getViewState().getString("SupplierCode"));
		}
	 ...
	 */
	 public void page_DlgBack(ActionEvent e) throws Exception
	 {
		...
		// 仕入先コード
		if (!StringUtil.isBlank(param.getParameter(ListMasterSupplierBusiness.SUPPLIERCODE_KEY)))
		{
			txt_supplierCode.setText(param.getParameter(ListMasterSupplierBusiness.SUPPLIERCODE_KEY));
		}
		// カーソルを作業者コードに移動する
		setFocus(txt_workerCode);
	 }
	...
	public void btn_ConsignorSearch_Click(ActionEvent e) throws Exception
	{
		...
		// 検索条件
		param.setParameter(ListMasterConsignorBusiness.SEARCH_KEY, searchKey);
	 ...
	 * @throws Exception 全ての例外を報告します。 
	 */
	public void btn_SupplierSearch_Click(ActionEvent e) throws Exception
	{
		// 仕入先検索画面へ検索条件をセットする
		ForwardParameters param = new ForwardParameters();

		// 荷主コード
		param.setParameter(ListMasterSupplierBusiness.CONSIGNORCODE_KEY, txt_consignorCode.getText());
		// 仕入先コード
		param.setParameter(ListMasterSupplierBusiness.SUPPLIERCODE_KEY, txt_supplierCode.getText());
		// 処理中画面->結果画面
	 ...
	 * <BR>
	 * 概要:該当仕入先マスタ情報の修正・削除画面を表示します。<BR>
     * <BR>
     * <DIR>
	...
	public void btn_next_Click(ActionEvent e) throws Exception
	{
		...
		try
		{
            ...
            if (schedule.nextCheck(conn,param))
            {
				...
				// ViewStateに仕入先コードをセットする
				getViewState().setString("SupplierCode", txt_supplierCode.getText());
				// ViewStateに画面タイトルをセットする
				getViewState().setString("TITLE", lbl_SettingName.getResourceKey());
                // 画面遷移を行う
				forward("/master/correctsuppliermaster/CorrectSupplierMaster2.do");
            }
            else
            {
