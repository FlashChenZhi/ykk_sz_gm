 ...
 * @author  $Author: mori $
 */
public class SessionIdmStockRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionIdmStockRet(Connection conn, IdmControlParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	...
	{	
		...
		// オーダー順をセット
		stKey.setConsignorCodeOrder(1, true);
		stKey.setConsignorNameOrder(2,true);
		stKey.setItemCodeOrder(3, true);
		stKey.setItemName1Order(4,true);
		stKey.setCasePieceFlagOrder(5,true);
		stKey.setUseByDateOrder(6,true);
	...
	private IdmControlParameter[] convertToStockParams(Stock[] stock)
	{
		...
		for (int i = 0; i < stock.length; i++)
		{
			...
			// ケース/ピース区分：「ピース」の場合
			if (stock[i].getCasePieceFlag().equals(Stock.CASEPIECE_FLAG_PIECE))
			{
				// 在庫ケース数
