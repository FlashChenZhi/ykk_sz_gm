 ...
 * <BR>
 * ケースITF一覧リストボックス（移動ラック）用のデータを検索するためのクラスです。<BR>
 * 検索条件をパラメータとして受け取り、ケース一覧の検索を行います。<BR>
 ...
 * @author  $Author: mori $
 */
public class SessionIdmCaseItfRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionIdmCaseItfRet(Connection conn, IdmControlParameter stParam) throws Exception
	{
		wConn = conn ;		
		find(stParam) ;
	}
