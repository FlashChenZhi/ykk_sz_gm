...
import jp.co.daifuku.wms.sorting.report.SortingResultWriter;

/**
 * Designer : K.Toda <BR>
 * Maker : K.Toda <BR>
 * <BR>
 * 仕分実績リスト発行処理を行うためのクラスです。 <BR>
 * 画面から入力された内容をパラメータとして受け取り、仕分実績リスト発行処理を行います。 <BR>
 * このクラスでは以下の処理を行います。 <BR>
 * <BR>
 * 1.初期表示処理(<CODE>initFind()</CODE>メソッド) <BR> 
 * <BR>
 * <DIR>
 *   仕分実績情報<CODE>(ResultView)</CODE>に荷主コードが1件のみ存在した場合、該当する荷主コードを返します。 <BR> 
 ...
 *   <CODE>SortingResultWriter</CODE>クラスを使用して仕分実績リストの印刷処理を行います。<BR>
 *   <BR>
 *   ＜パラメータ＞ *必須入力 <BR>
 *   <BR>
 *   <DIR>
 *   荷主コード* : ConsignorCode <BR>
 *   仕分実績日 : SortingDate <BR>
 *   商品コード : ItemCode <BR>
 ...
 * @author  $
 */
public class SortingQtyListSCH extends AbstractSortingSCH
{
	...
	public Parameter initFind(Connection conn, Parameter searchParam) throws ReadWriteException, ScheduleException
	{
		...
		int count = resultviewHandler.count(searchKey);
	...
	/**
	 * 画面から入力された内容をパラメータとして受け取り、 <BR>
	 * SortingResultWriterクラスを使用して仕分実績リストの印刷処理を行います。 <BR>    
	...
	public boolean startSCH(Connection conn, Parameter[] startParams) throws ReadWriteException, ScheduleException
	{
		try
		{
			...
			// 印刷クラスを作成します
			SortingResultWriter writer = createWriter(conn, sortingParameter);
			// 印刷を開始します
		...
		{
			// エラーをログファイルに書き込む
			RmiMsgLogClient.write(new TraceHandler(6027005, e), "SortingQtyListSCH");
	...
	public int count(Connection conn, Parameter countParam) throws ReadWriteException, ScheduleException
	{
		...
		SortingResultWriter writer = createWriter(conn, param);
		// 対象件数を取得する
	 ...
	 * @return 印刷処理クラス
	 */
	protected SortingResultWriter createWriter(Connection conn, SortingParameter parameter)
	{
		// 印刷処理クラスを生成する
		SortingResultWriter writer = new SortingResultWriter(conn);
		
		// 荷主コード
		writer.setConsignorCode(parameter.getConsignorCode());
		// 仕分実績日
		writer.setSortingDate(parameter.getSortingDate());
		// 商品コード
		writer.setItemCode(parameter.getItemCode());
		// ケースピース区分
		writer.setCasePieceFlag(parameter.getCasePieceFlag());
		// クロス／ＤＣ区分
