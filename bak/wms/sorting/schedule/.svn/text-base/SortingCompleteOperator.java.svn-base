 ...
 * Maker : T.Hondo <BR>
 * <BR>
 * 仕分完了処理を行うためのクラスです。<BR>
 * 作業No.、作業区分、処理名、作業者コード、作業者名、端末No.を受け取り、仕分完了処理を行います。(<CODE>complete()</CODE>メソッド) <BR>
 ...
 * @author  $Author: mori $
 */
public class SortingCompleteOperator extends AbstractSortingSCH
{
	//	Class variables -----------------------------------------------
	 ...
	 * このクラスの初期化を行ないます。
	 */
	public SortingCompleteOperator()
	{
		wMessage = null;
	}
	...
	public boolean complete(Connection conn, String jobno, String jobtype, String pname) throws ReadWriteException, ScheduleException
	{
		try
		{
			...
			else
			{
				...
				RmiMsgLogClient.write("6006040" + wDelim + jobno, "SortingCompleteOperator");
	...
	private boolean updateStock(Connection conn, String stockid, String pname, int resultqty, int totalqty) throws ReadWriteException, ScheduleException
	{
		try
		{
			...
			else
			{
				...
				RmiMsgLogClient.write("6006040", "SortingCompleteOperator");
