 ...
 * <BR>
 * 1.検索処理(<CODE>SessionSortingWorkInfoSupplierRet(Connection conn,SortingParameter param)</CODE>メソッド)<BR>
 * <DIR>
...
public class SessionSortingWorkInfoSupplierRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionSortingWorkInfoSupplierRet(Connection conn, SortingParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	...
	private SortingParameter[] convertToSortingSupportParams(WorkingInformation[] workInfo)
	{
		...
		for (int i = 0; i < workInfo.length; i++)
		{
			...
			stParam[i].setSupplierCode(workInfo[i].getSupplierCode());
			stParam[i].setSupplierName(workInfo[i].getSupplierName1());
