 ...
 * <BR>
 * 1.検索処理(<CODE>SessionSortingPlanDateRet(Connection conn,SortingParameter param)</CODE>メソッド)<BR>
 * <DIR>
 ...
 * @author  $Author: mori $
 */
public class SessionSortingPlanDateRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionSortingPlanDateRet(Connection conn, SortingParameter stParam) throws Exception
	{
		wConn = conn;
		find(stParam);
	}
	...
	public Parameter[] getEntities()
	{
		...
		SortingPlan[] instock = null;
		if (wLength > 0 && wLength <= DatabaseFinder.MAXDISP)
		{
			try
			{
				instock = (SortingPlan[]) wFinder.getEntities(wStartpoint, wEndpoint);
				resultArray = (SortingParameter[]) convertToSortingParams(instock);
			}
			catch (Exception e)
			{
	 ...
	 * 
	 * @param ety 仕分予定情報
	 * @return Parameter[] 仕分予定情報をセットした<CODE>SortingParameter</CODE>クラス
	 */
	private Parameter[] convertToSortingParams(Entity[] ety)
	{

		SortingPlan[] sortingPlan = (SortingPlan[]) ety;
		...
		for (int i = 0; i < sortingPlan.length; i++)
		{
			...
			{
				stParam[i].setPlanDate(sortingPlan[i].getPlanDate()); //入庫予定日
			}
		}
