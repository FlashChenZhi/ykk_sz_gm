// $Id: ListShippingPlanListBusiness.java,v 1.1.1.1 2006/08/17 09:34:28 mori Exp $
 ...
 */
package jp.co.daifuku.wms.shipping.display.web.listbox.listshippingplanlist;
...
import jp.co.daifuku.wms.shipping.display.web.listbox.sessionret.SessionShippingPlanListRet;
 ...
 * @author  $Author: mori $
 */
public class ListShippingPlanListBusiness extends ListShippingPlanList implements WMSConstants
{
	// Class fields --------------------------------------------------
	...
	public void page_Load(ActionEvent e) throws Exception
	{
		...
		// 作業状態
		String statusflag = request.getParameter(ListShippingItemBusiness.WORKSTATUSITEM_KEY);
		// 荷主コードの必須入力チェック、禁止文字チェック
		if (!WmsCheckker
			.consignorCheck(consignorcode, lst_ShippingPlanList, pgr_U, pgr_D, lbl_InMsg))
		{
			return;
		}
		...
		if (!WmsCheckker
			.rangePlanDateCheck(
				startshippingplandate,
				endshippingplandate,
				lst_ShippingPlanList,
				pgr_U,
				pgr_D,
				lbl_InMsg))
		{
		...
		// 出荷先コードの禁止文字チェック
		if (!WmsCheckker.charCheck(customercode, lst_ShippingPlanList, pgr_U, pgr_D, lbl_InMsg))
		{
			return;
		}

		// 伝票No.の禁止文字チェック
		if (!WmsCheckker.charCheck(ticketnumber, lst_ShippingPlanList, pgr_U, pgr_D, lbl_InMsg))
		{
			return;
		}

		// 商品コードの禁止文字チェック
		if (!WmsCheckker.charCheck(itemcode, lst_ShippingPlanList, pgr_U, pgr_D, lbl_InMsg))
		{
			return;
		}
		...
		// SessionShippingPlanListRet インスタンス生成
		SessionShippingPlanListRet listbox = new SessionShippingPlanListRet(conn, param);
		// Sessionにlistboxを保持
	 ...
	 * @throws Exception 全ての例外を報告します。
	 */
	private void setList(SessionShippingPlanListRet listbox, String actionName) throws Exception
	{
		// ロケールを取得
		...
		if (len > 0)
		{
			...
			{
				// 最終行を取得
				int count = lst_ShippingPlanList.getMaxRows();
				// 行追加
				lst_ShippingPlanList.addRow();

				// 最終行に移動
				lst_ShippingPlanList.setCurrentRow(count);
				lst_ShippingPlanList.setValue(
					1,
					WmsFormatter.toDispDate(sparam[i].getPlanDate(), locale));
				lst_ShippingPlanList.setValue(2, sparam[i].getCustomerCode());
				lst_ShippingPlanList.setValue(3, sparam[i].getShippingTicketNo());
				lst_ShippingPlanList.setValue(4, sparam[i].getItemCode());
				lst_ShippingPlanList.setValue(5, WmsFormatter.getNumFormat(sparam[i].getEnteringQty()));
				lst_ShippingPlanList.setValue(6, WmsFormatter.getNumFormat(sparam[i].getPlanCaseQty()));
				lst_ShippingPlanList.setValue(7, WmsFormatter.getNumFormat(sparam[i].getResultCaseQty()));
				lst_ShippingPlanList.setValue(8, sparam[i].getStatusName());
				lst_ShippingPlanList.setValue(9, sparam[i].getCustomerName());
				lst_ShippingPlanList.setValue(10, Integer.toString(sparam[i].getShippingLineNo()));
				lst_ShippingPlanList.setValue(11, sparam[i].getItemName());
				lst_ShippingPlanList.setValue(
					12,
				WmsFormatter.getNumFormat(sparam[i].getBundleEnteringQty()));
				lst_ShippingPlanList.setValue(13, WmsFormatter.getNumFormat(sparam[i].getPlanPieceQty()));
				lst_ShippingPlanList.setValue(14, WmsFormatter.getNumFormat(sparam[i].getResultPieceQty()));

				// ToolTip用のデータを編集
				ToolTipHelper toolTip = new ToolTipHelper();
				toolTip.add(title_CustomerName, sparam[i].getCustomerName());
				toolTip.add(title_ItemName, sparam[i].getItemName());
				toolTip.add(title_StatusName, sparam[i].getStatusName());
				// TOOL TIPをセットする	
				lst_ShippingPlanList.setToolTip(count, toolTip.getText());
			}
		}
		else
		{
			...
			// ヘッダーを隠します
			lst_ShippingPlanList.setVisible(false);
			// エラーメッセージ表示
	 ...
	 * @throws Exception 
	 */
	public void lst_ShippingPlanList_EnterKey(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_ShippingPlanList_TabKey(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_ShippingPlanList_InputComplete(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_ShippingPlanList_ColumClick(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_ShippingPlanList_Server(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_ShippingPlanList_Change(ActionEvent e) throws Exception
	{
	}
	 ...
	 * @throws Exception 
	 */
	public void lst_ShippingPlanList_Click(ActionEvent e) throws Exception
	{
	}
	...
	{
		// Sessionにlistboxを保持
		SessionShippingPlanListRet listbox =
			(SessionShippingPlanListRet) this.getSession().getAttribute("LISTBOX");
