// $Id: ShippingListBusiness.java,v 1.1.1.1 2006/08/17 09:34:29 mori Exp $
 ...
 */
package jp.co.daifuku.wms.shipping.display.web.shippinglist;
...
import jp.co.daifuku.wms.shipping.display.web.listbox.listshippinglist.ListShippingListBusiness;
...
import jp.co.daifuku.wms.shipping.schedule.ShippingListSCH;
 ...
 * <BR>
 * 出荷作業リスト発行画面クラスです。<BR>
 * 出荷作業リスト発行処理を行うスケジュールにパラメータを引き渡します。<BR>
 ...
 * <TR><TD>2004/08/09</TD><TD>Y.Okamura</TD><TD>created this class</TD></TR>
 * </TABLE>
 * <BR>
 * @version $Revision: 1.1.1.1 $, $Date: 2006/08/17 09:34:29 $
 * @author  $Author: mori $
 */
public class ShippingListBusiness extends ShippingList implements WMSConstants
{
	// Class fields --------------------------------------------------
	 ...
	 * 呼ばれます。Pageに定義されているpage_ConfirmBackをオーバライドします。<BR>
	 * <BR>
	...
	public void page_ConfirmBack(ActionEvent e) throws Exception
	{
		...
		try
		{
			...
			// スケジュールを開始する
			WmsScheduler schedule = new ShippingListSCH();
	 ...
	 * <CODE>Page</CODE>クラスに定義されている<CODE>page_DlgBack</CODE>をオーバライドします。<BR>
	 * 
	 * @param e ActionEvent イベントの情報を格納するクラスです。
	 * @throws Exception 全ての例外を報告します。
	 */
	public void page_DlgBack(ActionEvent e) throws Exception
	{
		...
		// 開始出荷予定日
		Date strPlanDate = WmsFormatter.toDate(param.getParameter(ListShippingPlanDateBusiness.STARTSHIPPINGPLANDATE_KEY)
											,this.getHttpRequest().getLocale());
		// 終了出荷予定日
		Date endPlanDate = WmsFormatter.toDate(param.getParameter(ListShippingPlanDateBusiness.ENDSHIPPINGPLANDATE_KEY)
											,this.getHttpRequest().getLocale());
		// 出荷先コード
		...
		// 開始出荷予定日
		if (!StringUtil.isBlank(strPlanDate))
		{
			txt_StrtShpPlanDate.setDate(strPlanDate);
		}
		// 終了出荷予定日
		if (!StringUtil.isBlank(endPlanDate))
		{
			txt_EdShpPlanDate.setDate(endPlanDate);
		}
		// 出荷先コード
	 ...
	 * <BR>
	 *  <DIR>
	 *  入力エリアの項目を初期値に戻します。<BR>
	 *  このメソッドは初期化するための<CODE>page_Load(ActionEvent e)</CODE>メソッドを呼び出します。<BR>
	 *  </DIR>
	 * 
	 * @param e ActionEvent イベントの情報を格納するクラスです。
	 * @throws Exception 全ての例外を報告します。
	 ...
	 * <BR>
	 * 概要：入力エリアの入力項目をパラメータにセットし別画面で出荷作業一覧リストボックスを表示します。<BR>
	 * 本メソッドでは以下の処理を行います。<BR>
	 * <BR>
	 * <DIR>
	 * 1.入力項目をパラメータにセットする。<BR>
	 * 2.出荷作業一覧リストボックスを表示する。<BR>
	 * </DIR>
	 * 
	 * @param e ActionEvent イベントの情報を格納するクラスです。
	 * @throws Exception 全ての例外を報告します。
	...
	public void btn_PDisplay_Click(ActionEvent e) throws Exception
	{
		...
		if(pul_WorkStatus.getSelectedIndex() == 0)
		{
			forwardParam.setParameter(ListShippingListBusiness.WORKSTATUS_KEY,
		...
		// 作業一覧検索リストボックスを表示する。
		redirect(
			"/shipping/listbox/listshippinglist/ListShippingList.do",
			forwardParam,
			"/progress.do");
