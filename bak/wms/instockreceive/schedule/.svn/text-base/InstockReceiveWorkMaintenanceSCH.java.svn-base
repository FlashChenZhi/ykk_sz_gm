 ...
 * Maker : S.Yoshida  <BR>
 * <BR>
 * WEBクロス/DC入荷作業メンテナンスを行うためのクラスです。 <BR>
 * 画面から入力された内容をパラメータとして受け取り、クロス/DC入荷作業メンテナンス処理を行います。 <BR>
 ...
 *   仕入先コード、商品コード、状態フラグ、クロス/DC、伝票No.、行No.順に表示を行います。 <BR>
 * <BR>
 *   ＜検索条件＞ <BR><DIR>
 *     状態フラグ：削除(9)以外 <BR>
 *     TC/DC区分：TC(1)以外 <BR>
 ...
 *   ためうちエリアに表示されている内容をパラメータとして受け取り、入荷作業メンテナンス処理を行います。 <BR>
 ...
 *       4.最終更新処理名を更新する。 <BR>
 * <BR>
 *     -在庫情報テーブル(DNSTOCK)の更新（入荷完了処理の中で行う。） 注）完了→完了への変更時は在庫情報は変更しない。 <BR>
 ...
 * @author  $Author: mori $
 */
public class InstockReceiveWorkMaintenanceSCH extends AbstractInstockReceiveSCH
{
	 ...
	 * 処理名
	 */
	protected static final String wProcessName = "InstockWorkMaintenance";
	 ...
	 * このクラスの初期化を行ないます。
	 */
	public InstockReceiveWorkMaintenanceSCH()
	{
		wMessage = null;
	}
	...
	public Parameter[] query(Connection conn, Parameter searchParam) throws ReadWriteException, ScheduleException
	{
		...
		// TC/DC区分
		if (wParam.getTcdcFlag().equals(InstockReceiveParameter.TCDC_FLAG_CROSSTC))
		{
			// クロス
			wKey.setTcdcFlag(WorkingInformation.TCDC_FLAG_CROSSTC);
			wNameKey.setTcdcFlag(WorkingInformation.TCDC_FLAG_CROSSTC);
		}
		else if (wParam.getTcdcFlag().equals(InstockReceiveParameter.TCDC_FLAG_DC))
		{
			// DC
			wKey.setTcdcFlag(WorkingInformation.TCDC_FLAG_DC);
			wNameKey.setTcdcFlag(WorkingInformation.TCDC_FLAG_DC);
		}

		// 仕入先コード、商品コード、状態フラグ、クロス/DC、伝票No.、行No.順でソート順をセット
		wKey.setSupplierCodeOrder(1, true);
		wKey.setItemCodeOrder(2, true);
		wKey.setStatusFlagOrder(3, true);
		wKey.setTcdcFlagOrder(4, false);
		wKey.setInstockTicketNoOrder(5, true);
		wKey.setInstockLineNoOrder(6, true);

		// 該当する作業情報のデータ件数を取得するし、表示件数チェックを行います。
		...
		for (int i = 0; i < wWorkinfo.length; i++)
		{
			...
			if (wWorkinfo[i].getTcdcFlag().equals(WorkingInformation.TCDC_FLAG_CROSSTC))
			{
				...
				wTemp.setTcdcName(DisplayUtil.getTcDcValue(InstockReceiveParameter.TCDC_FLAG_CROSSTC));
			}
			else if (wWorkinfo[i].getTcdcFlag().equals(WorkingInformation.TCDC_FLAG_DC))
			{
				...
				wTemp.setTcdcName(DisplayUtil.getTcDcValue(InstockReceiveParameter.TCDC_FLAG_DC));
			}
			// 実績報告フラグ
			...
			wTemp.setInstockPlanUkey(wWorkinfo[i].getPlanUkey());
	...
	/**
	 * 画面から入力された内容をパラメータとして受け取り、クロス/DC入荷作業メンテナンスのスケジュールを開始します。<BR>
	 * 作業中のデータを更新した場合で、正常に処理が終了した場合はRFT作業情報の応答電文項目をNULLに更新します。<BR>
	 * また、作業中データ保存情報を検索し、該当データが存在した場合はそれを削除します。
	...
	public Parameter[] startSCHgetParams(Connection conn, Parameter[] startParams) throws ReadWriteException, ScheduleException
	{
		try
		{
			...
			String wRowNo = checkResultQty(conn, wParam);
			if(!StringUtil.isBlank(wRowNo))
			{
				// 6023374=No.{0} 入荷実績数の合計が{1}を超えた為、修正登録できませんでした。
				wMessage = "6023374" + wDelim + wRowNo + wDelim + "999,999";
			    return null;
			}
			...
			for (int i = 0; i < wParam.length; i ++)
			{
				...
				if (wWorkinfo != null)
				{
					...
					||   wWorkinfo.getStatusFlag().equals(WorkingInformation.STATUS_FLAG_NOWWORKING)))
					{
						...
						{
							int completeQty = (wParam[i].getEnteringQty() * wParam[i].getResultCaseQty()) + wParam[i].getResultPieceQty();
							int shortageQty = wWorkinfo.getPlanEnableQty() - completeQty;
							crossDocOperator.complete(conn,wWorkinfo.getPlanUkey(),completeQty,shortageQty);
						}
			...
			for (int i = 0; i < wParam.length; i++)
			{
				...
				if (wWorkinfo != null)
				{
					...
					if (wResultQty > 0 && wParam[0].getButtonType().equals(InstockReceiveParameter.BUTTON_MODIFYSUBMIT))
					{
						...
						// 完了→完了の場合、入荷数＜＝作業可能数－保留数 かつ TC/DC区分！＝「DC」チェック
						if (wParam[i].getStatusFlagL().equals(InstockReceiveParameter.STATUS_FLAG_COMPLETION)
						&&  wWorkinfo.getStatusFlag().equals(WorkingInformation.STATUS_FLAG_COMPLETION)
						&&  !wWorkinfo.getTcdcFlag().equals(WorkingInformation.TCDC_FLAG_DC)
												  ...
												  InstockReceiveParameter inputParam) throws ReadWriteException, ScheduleException
	{
		...
		try
		{
			...
			// 作業情報の検索結果を取得する。
			workinfo = (WorkingInformation) wObj.findPrimary(wKey);

			if (workinfo != null)
			{
				// デッドロックを防止するために予定情報に紐づく他の作業情報をロックする。
				wKey.KeyClear();
				wKey.setPlanUkey(workinfo.getPlanUkey());
				wKey.setJobNo(param.getJobNo(), "!=");
				...
				// 更新値を設定する。
				if (!workinfo.getStatusFlag().equals(WorkingInformation.STATUS_FLAG_COMPLETION))
				{
					// 開始済みに変更
					...
					else if (param.getStatusFlagL().equals(InstockReceiveParameter.STATUS_FLAG_UNSTARTED))
					{
						...
						wAKey.updateCollectJobNo(workinfo.getJobNo());
						wAKey.updateWorkerCode("");
						wAKey.updateWorkerName("");
						wAKey.updateTerminalNo("");
				...
				if (param.getStatusFlagL().equals(InstockReceiveParameter.STATUS_FLAG_COMPLETION))
				{
					...
					wAKey.updateShortageCnt(workinfo.getPlanEnableQty() - wTotalResultQty - workinfo.getPendingQty());
					// 賞味期限
	...
	// (*)
	/**
	 * 実績数が最大値を超えていないかチェックします。
	 * @param conn データベースとのコネクションを保持するインスタンス。
	 * @param irParam  画面から入力された内容を持つInstockReceiveParameterクラスのインスタンス配列。
	 * @return 最大値を超えていない場合はnull。超えている場合は、その行番号の文字列を返す。
	 * @throws ReadWriteException データベースとの接続で異常が発生した場合に通知されます。
	 * @throws ScheduleException 予期しない例外が発生した場合に通知されます。
	 */
	protected String checkResultQty(Connection conn, InstockReceiveParameter[] irParam) throws ReadWriteException, ScheduleException
	{
	    String instockPlanUkey = "";

	    InstockPlan resultPlan = null;
	    
	    Collection uniqueValues = new TreeSet();
	    for (int y = 0; y < irParam.length; y++)
		{
	        if(checkFlag)
	        {
	            return Integer.toString(irParam[y].getRowNo());
	        }
	        
	        if(!instockPlanUkey.equals(irParam[y].getInstockPlanUkey()))
	        {
	            // planukeyを渡して商品コード、仕入先コード、伝票No.、ラインNo.を取得します。
	            resultPlan = (InstockPlan)checkResultQuantity(conn,irParam[y]) ;
	        }
	        
	        //InstockPlanUkeyを更新
	        instockPlanUkey = irParam[y].getInstockPlanUkey();
	        
	        //実績を算出します。
	        long result = irParam[y].getEnteringQty()* irParam[y].getResultCaseQty() + irParam[y].getResultPieceQty();
			
	        uniqueValues.add(new TempObj(resultPlan.getItemCode(),resultPlan.getSupplierCode(),
                    resultPlan.getInstockTicketNo(),resultPlan.getInstockLineNo(),result));
	...
	/**
	 * planukeyを渡してINSTOCKPLANのエンティティクラスを取得します。
	 * @param conn データベースとのコネクションを保持するインスタンス。
	 * @param irParam  画面から入力された内容を持つInstockReceiveParameterクラスのインスタンス。
	 * @return INSTOCKPLANのエンティティクラスを返す。
	 * @throws ReadWriteException データベースとの接続で異常が発生した場合に通知されます。
	 * @throws ScheduleException 予期しない例外が発生した場合に通知されます。
	 */
	protected Entity checkResultQuantity(Connection conn, InstockReceiveParameter param)throws ReadWriteException, ScheduleException
	{
		InstockPlan planEntity = null ;
		InstockPlanHandler wHandler = new InstockPlanHandler(conn);
	    	    
		InstockPlanSearchKey sKey = new InstockPlanSearchKey();
	    
		sKey.setInstockPlanUkey(param.getInstockPlanUkey());
		...
		try
		{
			planEntity = (InstockPlan)wHandler.findPrimary(sKey);
		}
		catch(NoPrimaryException ne)
		{
			throw new ScheduleException(ne.getMessage());
		}
	...
	/**
	 * 一時的に商品データを保持しておく商品クラス。
	 */
	private class TempObj implements Comparable
	{
	    String itemCode = "";
	    String supplierCode = "";
	    String ticketNo = "";
		 ...
		 * パラメーター化されたコンストラクタです。
		 */
	    public TempObj(String itemCode, String supCode, String ticNo, int lineNo,long resultQty)
	    {	        
	        this.itemCode = itemCode;
	        this.supplierCode = supCode;
	        this.ticketNo = ticNo;
	        this.lineNo = lineNo;
		...
		/**
		 * compareToをオーバーライド
		 * 商品コード、仕入先コード、伝票番号、行番号が全て同じ場合、実績数を足し込みます。
		 * 足し込んだ結果、実績の最大値を超えた時は、checkFlagをtrueにします。
		 * @param obj TempObjクラスのインスタンス。
		 * @return 実績数を足し込んだ時は0を返す。それ以外は-1を返す
		 * 
		 */
	    //compareToをオーバーライド
	    ...
	    {    
	        ...
	        if(((TempObj)obj).itemCode.equals(itemCode) &&
	           ((TempObj)obj).supplierCode.equals(supplierCode) &&
	           ((TempObj)obj).ticketNo.equals(ticketNo) &&
	           (((TempObj)obj).lineNo==lineNo) )
	        {
	            
	            this.resultQty += ((TempObj)obj).resultQty;
	            if(resultQty > 999999)
	            {
	                checkFlag = true ;
	            }
	...
	// (*) このマークの箇所は、正常に機能していないと思われます。compareToは呼び出されないと思います。
