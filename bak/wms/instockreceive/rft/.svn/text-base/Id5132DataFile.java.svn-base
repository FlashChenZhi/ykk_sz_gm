// $Id: Id5132DataFile.java,v 1.1.1.1 2006/08/17 09:34:14 mori Exp $
...
/**
 * 仕入先単位入荷検品ファイル(ID5132.txt)を操作するためのクラスです。
 * 
 * <p>
 * <table border="1">
 * <CAPTION>ID5132のファイルの各行の構造</CAPTION>
 ...
 * <tr><td>集約作業No</td>		<td>16 byte</td><td>集約作業No</td></tr>
 ...
 * @author $Author: mori $
 */
public class Id5132DataFile extends WorkDataFile
{
	// Class fields----------------------------------------------------
	/**
	 * 仕入先コードのオフセットの定義
	 ...
	 * 伝票番号のオフセットの定義
	 */
	private static final int OFF_INSTOCK_TICKET_NO = OFF_SUPPLIER_CODE + LEN_SUPPLIER_CODE;
	 ...
	 * 集約作業Noのオフセットの定義
	 */
	private static final int OFF_COLLECT_JOB_NO = OFF_INSTOCK_TICKET_LINE_NO + LEN_TICKET_LINE_NO;
	 ...
	 * TC/DC区分のオフセットの定義
	 */
	private static final int OFF_TCDC_FLAG = OFF_QUALITY + LEN_QUALITY;
	 ...
	 * 分納区分：分納なし
	 */
	public static final String NOT_INSTALLMENT = "0";
	...
	/**
	 * 仕入先一括入荷検品データファイル名を表すフィールド
	 */
	static final String ANS_FILE_NAME = "ID5132.txt" ;
	 ...
	 */
	public Id5132DataFile(String filename)
	{
		super(filename);
            ...
            InvalidStatusException
    {
		...
		for (next(); currentLine != null; next())
		{
			...
			workinfo.setTcdcFlag(TcdcFlag.convertTcdc(getTcdcFlag()));
     ...
     * @throws InvalidStatusException	不正なTC/DC区分が入っていた場合に通知されます。
	 */
	public void write(Entity[] obj) throws IOException, InvalidStatusException
	{
		...
		for (int i = 0; i < workinfo.length; i ++)
		{
			...
			setTcdcFlag(TcdcFlag.convertTcdcBySupplier(workinfo[i].getTcdcFlag()));
			setInstockPlanQty(workinfo[i].getPlanEnableQty());
			setInstockResultQty(workinfo[i].getResultQty());
			setInstallmentDelivery(NOT_INSTALLMENT);
			setQuality(GOOD);
			setWorkSeconds(0);
	...
	/**
	 * データバッファからTC/DC区分を取得します。
	 * @return		作業情報のTC/DC区分
     * @throws InvalidStatusException	不正なTC/DC区分が入っていた場合に通知されます。
	 */
	public String getTcdcFlag() throws InvalidStatusException
	{
	    String tcdcBySupplier = getColumn(OFF_TCDC_FLAG, LEN_TCDC_FLAG);
		return TcdcFlag.convertTcdc(tcdcBySupplier);
	}
	...
	/**
	 * データバッファにTC/DC区分をセットします。
	 * @param	tcdc		作業情報のTC/DC区分
     * @throws InvalidStatusException	不正なTC/DC区分が渡された場合に通知されます。
	 */
	public void setTcdcFlag(String tcdc) throws InvalidStatusException
	{
	    String tcdcBySupplier = TcdcFlag.convertTcdcBySupplier(tcdc);
		setColumn(tcdcBySupplier, OFF_TCDC_FLAG, LEN_TCDC_FLAG);
	}
