 ...
 * <BR>
 * <B>1.検索処理(<CODE>SessionInstockReceiveSupplierPlanRet(Connection, InstockReceiveParameter)</CODE>メソッド)<BR></B>
 * <DIR>
 ...
 * @author  $Author: mori $
 */
public class SessionInstockReceiveSupplierPlanRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionInstockReceiveSupplierPlanRet(Connection conn, InstockReceiveParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	...
	private InstockReceiveParameter[] convertToInStockParams(InstockPlan[] instockPlan)
	{
		...
		for (int i = 0; i < instockPlan.length; i++)
		{
				...
				stParam[i].setSupplierCode(instockPlan[i].getSupplierCode());
				stParam[i].setSupplierName(instockPlan[i].getSupplierName1());
		}
