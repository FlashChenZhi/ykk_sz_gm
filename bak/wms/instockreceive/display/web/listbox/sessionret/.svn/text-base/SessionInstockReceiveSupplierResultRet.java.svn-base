 ...
 * <BR>
 * <B>1.検索処理(<CODE>SessionInstockReceiveSupplierResultRet(Connection, InstockReceiveParameter)</CODE>メソッド)<BR></B>
 * <DIR>
...
public class SessionInstockReceiveSupplierResultRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionInstockReceiveSupplierResultRet(Connection conn, InstockReceiveParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	...
	private InstockReceiveParameter[] convertToInStockParams(ResultView[] instockVIew)
	{
		...
		{	
			...
			for (int i = 0; i < instockVIew.length; i++)
			{
					...
					stParam[i].setSupplierCode(instockVIew[i].getSupplierCode());
					stParam[i].setSupplierName(instockVIew[i].getSupplierName1());
