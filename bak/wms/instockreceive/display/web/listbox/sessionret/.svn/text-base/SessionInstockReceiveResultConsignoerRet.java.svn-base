 ...
 * <BR>
 * <B>1.検索処理(<CODE>SessionInstockReceiveResultConsignoerRet(Connection, InstockReceiveParameter)</CODE>メソッド)<BR></B>
 * <DIR>
...
public class SessionInstockReceiveResultConsignoerRet extends SessionRet
{
	// Class fields --------------------------------------------------
	 ...
	 */
	public SessionInstockReceiveResultConsignoerRet(Connection conn, InstockReceiveParameter param) throws Exception
	{
		this.wConn = conn;
		find(param);
	}
	...
	private InstockReceiveParameter[] convertToInStockParams(ResultView[] instockVIew)
	{
		...
		{	
			...
			for (int i = 0; i < instockVIew.length; i++)
			{
					...
					stParam[i].setConsignorCode(instockVIew[i].getConsignorCode());
					stParam[i].setConsignorName(instockVIew[i].getConsignorName());
