// $Id: TcInstockReceiveWriter.java,v 1.4 2006/12/13 08:56:43 suresh Exp $
 ...
 * Maker : H.Akiyama <BR>
 * 
 * <CODE>TcInstockReceiveWriter</CODE>クラスは、ＴＣ入荷作業リストの帳票用データファイルを作成し、印刷を実行します。<BR>
 * <CODE>TcInstockReceiveListSCH</CODE>クラスでセットされた条件にもとづき、作業情報を検索し、<BR>
 * 結果をＴＣ入荷作業リスト用の帳票ファイルとして作成します。<BR>
 * このクラスは、以下の処理を行います。<BR>
 * <BR>
 * 帳票用データファイル作成処理(<CODE>startPrint()</CODE>メソッド)<BR>
 *	<DIR>
 *	1.<CODE>TcInstockReceiveListSCH</CODE>クラスからセットされた条件で作業情報の件数を検索します。<BR>
 ...
 */
public class TcInstockReceiveWriter extends CSVWriter
{
	// Class fields --------------------------------------------------
	 ...
	 * 出荷先コード
	 */
	protected String wCustomerCode;
	...
	// Constructors --------------------------------------------------
	/**
	 * TcInstockReceiveWriter オブジェクトを構築します。<BR>
	 * コネクションとロケールのセットを行います。<BR>
	 * @param conn <CODE>Connection</CODE> データベースとのコネクションオブジェクト
	 */
	public TcInstockReceiveWriter(Connection conn)
	{
		super(conn);
	}
	...
	/**
	 * 状態フラグに値をセットします。
	 * @param arrayStatusFlag セットする状態フラグ
	 */
	...
	{
		WorkingInformationHandler workHandle = new WorkingInformationHandler(wConn);
		WorkingInformationSearchKey searchKey = new WorkingInformationSearchKey();
		// 検索条件をセットし、件数取得を行う
		setWorkInfoSearchKey(searchKey);
		return workHandle.count(searchKey);

	}
	
	/**
	 * ＴＣ入荷作業リスト用のCSVファイルを作成し、印刷実行クラスを呼び出します。<BR>
	...
	public boolean startPrint()
	{
		...
		WorkingInformationReportFinder reportFinder = new WorkingInformationReportFinder(wConn);
		...
		try
		{
			...
			if (getDisplayOrder().equals(InstockReceiveParameter.DISPLAY_ORDER_TICKET))
			{
				...
				winfoKey.setCustomerCodeOrder(4, true);
				winfoKey.setInstockTicketNoOrder(5, true);
				winfoKey.setInstockLineNoOrder(6, true);
				winfoKey.setItemCodeOrder(7, true);
				winfoKey.setUseByDateOrder(8, true);
			}
			if (getDisplayOrder().equals(InstockReceiveParameter.DISPLAY_ORDER_ITEM))
			{
				...
				winfoKey.setSupplierCodeOrder(3, true);
				winfoKey.setCustomerCodeOrder(4, true);
				winfoKey.setItemCodeOrder(5, true);
				winfoKey.setInstockTicketNoOrder(6, true);
				winfoKey.setInstockLineNoOrder(7, true);
				winfoKey.setUseByDateOrder(8, true);
			}

			// データがない場合は印刷処理を行わない。
			if (reportFinder.search(winfoKey) <= 0)
			{
			...
			// 出力ファイルを作成します。
			if (!createPrintWriter(FNAME_INSTOCK_INSPECTION_TC))
			{
				return false;
			}
            
            // 出力ファイルにヘッダーを作成する。
            wStrText.append(HEADER_INSTOCK_INSPECTION_TC);
			...
			{
				workInfo = (WorkingInformation[]) reportFinder.getEntities(100);
			...
			// UCXSingleを実行。（印刷実行）
			if (!executeUCX(JOBID_INSTOCK_INSPECTION_TC))
			{
	...
	{
		// 検索キーをセットする。
		// ＴＣ入荷作業リスト発行画面からの呼び出し
		// 荷主コード、入荷予定日、仕入先コード、出荷先コード、作業状態区分(配列)
		// 荷主コード
